...
Puede proporcionar instrucciones personalizadas a codetutor creando un archivo `AGENTS.md`. Esto es similar a las reglas del Cursor. Contiene instrucciones que se incluirán en el contexto de LLM para personalizar su comportamiento para su proyecto específico.
...
Esto escaneará su proyecto y todo su contenido para comprender de qué se trata y generar un archivo `AGENTS.md` con él. Esto ayuda a codetutor a navegar mejor por el proyecto.
...
codetutor también admite la lectura del archivo `AGENTS.md` desde múltiples ubicaciones. Y esto tiene diferentes propósitos.
...
Para los usuarios que migran desde Claude Code, CodeTutor admite las convenciones de archivos de Claude Code como alternativas:
...
Puede especificar archivos de instrucciones personalizados en su `codetutor.json` o el `~/.config/opencode/codetutor.json` global. Esto le permite a usted y a su equipo reutilizar las reglas existentes en lugar de tener que duplicarlas en AGENTS.md.
...
Si bien codetutor no analiza automáticamente las referencias de archivos en `AGENTS.md`, puedes lograr una funcionalidad similar de dos maneras:

### Usar codetutor.json

El enfoque recomendado es utilizar el campo `instructions` en `codetutor.json`:
...
Puede enseñarle a codetutor a leer archivos externos proporcionando instrucciones explícitas en su `AGENTS.md`. He aquí un ejemplo práctico:
...
- Asegúrese de que codetutor cargue archivos solo cuando sea necesario para la tarea específica

:::tip
Para monorepos o proyectos con estándares compartidos, usar `codetutor.json` con patrones globales (como `packages/*/AGENTS.md`) es más fácil de mantener que las instrucciones manuales.
:::
