---
title: Providers
description: Using any LLM provider in CodeTutor.
---

import config from "../../../config.mjs"
export const console = config.console

CodeTutor uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your CodeTutor config.
...
You can customize the providers through the `provider` section in your CodeTutor
config.
...
OpenCode Zen is an optional third-party provider whose model catalog works with CodeTutor.
[Learn more, including the non-affiliation notice](/docs/zen).

:::tip
If you are new, we recommend starting with OpenCode Zen.
:::

1. Run the `/connect` command in the TUI, select `OpenCode Zen`, and head to [opencode.ai/zen](https://opencode.ai/zen).
...
It works like any other provider in CodeTutor and is completely optional to use.

---

## CodeTutor Go

CodeTutor Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the CodeTutor team that have been
tested and verified to work well with CodeTutor.

1. Run the `/connect` command in the TUI, select `CodeTutor Go`, and head to [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/zen).
...
To use Amazon Bedrock with CodeTutor:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `codetutor.json`:

   ```json title="codetutor.json"
   {
     "$schema": "https://codetutor-docs.vercel.app/config.json",
     "provider": {
       "amazon-bedrock": {
...
CodeTutor. Anthropic explicitly prohibits this.

Previous versions of CodeTutor came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in CodeTutor with zero setup:
...
You can configure codetutor to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="codetutor.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://codetutor-docs.vercel.app/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for codetutor to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your codetutor config.
...
CodeTutor supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; CodeTutor uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize CodeTutor. Sign in and approve.

   :::note
   CodeTutor requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, CodeTutor surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — CodeTutor forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in CodeTutor is experimental. Features, configuration, and
behavior may change in future releases.
:::

CodeTutor integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in codetutor auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
CodeTutor uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock CodeTutor
to only use your own GitLab-hosted instance, add the following to your
`codetutor.json` file. It is also recommended to disable session sharing.
...
Customize through `codetutor.json`:
...
When a `duo-workflow-*` model is selected, CodeTutor will:
...
4. Route tool execution requests through CodeTutor's permission-gated tool system
...
To use GMI Cloud with CodeTutor:
...
To use Google Vertex AI with CodeTutor:
...
```jsonc title="~/.config/opencode/codetutor.jsonc"
{
  "$schema": "https://codetutor-docs.vercel.app/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each CodeTutor conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each CodeTutor conversation listed as a separate session.
...
You can configure codetutor to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="codetutor.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://codetutor-docs.vercel.app/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure codetutor to use local models through LM Studio.

```json title="codetutor.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://codetutor-docs.vercel.app/config.json",
  "provider": {
    "lmstudio": {
...
You can configure codetutor to use local models through Ollama.

:::tip
Ollama can automatically configure itself for CodeTutor. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::

```json title="codetutor.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://codetutor-docs.vercel.app/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with CodeTutor:
...
3. Copy the API key for use in CodeTutor.
...
6. **Important**: Before using cloud models in CodeTutor, you must pull the model information locally:
...
OpenCode Zen is a list of tested and verified models provided by the CodeTutor team. [Learn more](/docs/zen).
...
3. Enter your CodeTutor API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your codetutor config.
...
5. You can also customize them through your codetutor config. Here's an example of specifying a provider
...
5. Complete sign-in in the browser that opens. CodeTutor captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
The model catalog is provided automatically. A minimal `codetutor.json` is all that's needed:
...
You can also customize models through your codetutor config. Here's an example of specifying provider routing order.
...
2. Select **SuperGrok Subscription**. CodeTutor opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by CodeTutor. CodeTutor polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
CodeTutor refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in codetutor.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `codetutor.json` file in your project directory:

   ```json title="codetutor.json" ""myprovider"" {5-15}
   {
     "$schema": "https://codetutor-docs.vercel.app/config.json",
     "provider": {
       "myprovider": {
...
```json title="codetutor.json" {9,11,17-20}
{
  "$schema": "https://codetutor-docs.vercel.app/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow CodeTutor to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `codetutor auth list` to see if the credentials
...
2. For custom providers, check the codetutor config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your codetutor config.
