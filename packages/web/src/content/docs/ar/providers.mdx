---
title: المزوّدون
description: استخدام أي موفّر LLM في CodeTutor.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم CodeTutor ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات CodeTutor.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات CodeTutor.
...
OpenCode Zen هي قائمة نماذج يوفّرها فريق CodeTutor وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع CodeTutor. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع OpenCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر opencode، ثم انتقل إلى [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/auth).
...
يعمل مثل أي مزوّد آخر في CodeTutor واستخدامه اختياري بالكامل.

---

## CodeTutor Go

CodeTutor Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق CodeTutor، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع CodeTutor.

1. شغّل الأمر `/connect` في TUI، واختر `CodeTutor Go`، ثم انتقل إلى [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/zen).
...
لاستخدام Amazon Bedrock مع CodeTutor:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `codetutor.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل CodeTutor غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم CodeTutor نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر CodeTutor
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`codetutor.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام Google Vertex AI مع CodeTutor:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة CodeTutor تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة CodeTutor مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع CodeTutor. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع CodeTutor:
...
3. انسخ مفتاح API لاستخدامه في CodeTutor.
...
6. **مهم**: قبل استخدام نماذج السحابة في CodeTutor، يجب سحب معلومات النموذج محليا:
...
OpenCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق CodeTutor. [اعرف المزيد](/docs/zen).
...
3. أدخل مفتاح API الخاص بـ CodeTutor.
...
4. أنشئ أو حدّث ملف `codetutor.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ CodeTutor بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `codetutor auth list` لمعرفة ما إذا كانت بيانات الاعتماد
