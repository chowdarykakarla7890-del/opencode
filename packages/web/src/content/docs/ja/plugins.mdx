---
title: プラグイン
description: CodeTutor を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで CodeTutor を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、CodeTutor のデフォルトの動作を変更したりできます。
...
- `.codetutor/plugins/` - プロジェクトレベルのプラグイン
...
1. グローバル設定 (`~/.config/opencode/codetutor.json`)
2. プロジェクト設定 (`codetutor.json`)
3. グローバルプラグインディレクトリ (`~/.config/opencode/plugins/`)
4. プロジェクトプラグインディレクトリ (`.codetutor/plugins/`)
...
CodeTutor は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための CodeTutor SDK クライアント。
...
ここでは、CodeTutor を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://codetutor-docs.vercel.app/docs/sdk) を参照してください。
...
CodeTutor が `.env` ファイルを読み取らないようにします。
...
プラグインは CodeTutor にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、CodeTutor が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して CodeTutor で使用できます。
