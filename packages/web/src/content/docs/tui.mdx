---
title: TUI
description: Using the CodeTutor terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodeTutor provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running CodeTutor starts the TUI for the current directory.
...
When using the CodeTutor TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to CodeTutor. Allows you to select from available providers and add their API keys.
...
Exit CodeTutor. _Aliases_: `/quit`, `/q`
...
This is separate from `codetutor.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long CodeTutor waits after the leader key. Defaults to `2000`.
...
Use `CODETUTOR_TUI_CONFIG` to load a custom TUI config path.
