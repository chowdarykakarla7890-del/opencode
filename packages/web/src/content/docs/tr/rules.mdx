---
title: Kurallar
description: codetutor için özel talimatları ayarlayın.
---

Bir `AGENTS.md` dosyası oluşturarak codetutor için özel talimatlar verebilirsiniz. Bu, Cursor rules yaklaşımına benzer. Özel projeniz için davranışını özelleştirmek amacıyla LLM'in içeriğine dahil edilecek talimatları içerir.
...
codetutor ayrıca `AGENTS.md` dosyasının birden fazla konumdan okunmasını da destekler. Ve bu farklı amaçlara hizmet ediyor.
...
Ayrıca `~/.config/opencode/AGENTS.md` dosyasında genel kurallara sahip olabilirsiniz. Bu, tüm codetutor oturumlarına uygulanır.
...
Özel talimat dosyalarını `codetutor.json` veya global `~/.config/opencode/codetutor.json`'de belirtebilirsiniz. Bu, sizin ve ekibinizin, mevcut kuralları AGENTS.md'ye kopyalamak yerine yeniden kullanmasına olanak tanır.
...
### codetutor.json'u kullanma

Önerilen yaklaşım, `instructions`'deki `codetutor.json` alanını kullanmaktır:
...
:::tip
Monorepo'lar veya ortak standartlara sahip projeler için, `codetutor.json`'yi glob desenleriyle (`packages/*/AGENTS.md` gibi) kullanmak, manuel talimatlara göre daha sürdürülebilirdir.
:::
