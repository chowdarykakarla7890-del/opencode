...
تتيح مهارات الوكيل لـ CodeTutor اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث CodeTutor في هذه المواقع:

- إعدادات المشروع: `.codetutor/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج CodeTutor صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.codetutor/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.
...
أنشئ `.codetutor/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض CodeTutor المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `codetutor.json`:
...
**للوكلاء المدمجين** (في `codetutor.json`):
