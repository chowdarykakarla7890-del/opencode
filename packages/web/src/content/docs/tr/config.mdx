...
codetutor hem **JSON** hem de **JSONC** (JSON Yorumlarla birlikte) formatlarını destekler.
...
2. **Genel yapılandırma** (`~/.config/opencode/codetutor.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`CODETUTOR_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `codetutor.json`) - projeye özgü ayarlar
5. **`.codetutor` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`CODETUTOR_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
:::note
`.codetutor` ve `~/.config/opencode` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
Global codetutor yapılandırmanızı `~/.config/opencode/codetutor.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `codetutor.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.

:::tip
Projeye özel yapılandırmayı projenizin köküne yerleştirin.
:::

codetutor başlatıldığında geçerli dizinde bir yapılandırma dosyası arar veya en yakın Git dizinine gider.
...
`CODETUTOR_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`CODETUTOR_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
ortam değişkeni. Bu dizin agent'lar, komutlar için aranacaktır.
modlar ve eklentiler tıpkı standart `.codetutor` dizini gibi olmalıdır ve
...
Özel dizin, genel yapılandırma ve `.codetutor` dizinlerinden sonra yüklenir, böylece bunların ayarlarını **geçersiz kılabilir**.

---

## Şema

Yapılandırma dosyası [**`codetutor-docs.vercel.app/config.json`**](https://codetutor-docs.vercel.app/config.json)'da tanımlanan bir şemaya sahiptir.
...
`codetutor serve` ve `codetutor web` komutları için sunucu ayarlarını `server` seçeneği aracılığıyla yapılandırabilirsiniz.
...
- `mdns` - mDNS hizmet bulmayı etkinleştirin. Bu, ağdaki diğer cihazların codetutor sunucunuzu keşfetmesine olanak tanır.
- `mdnsDomain` - mDNS hizmeti için özel alan adı. Varsayılan olarak `codetutor.local` şeklindedir. Aynı ağda birden fazla örneği çalıştırmak için kullanışlıdır.
...
codetutor yapılandırmanızda kullanmak istediğiniz sağlayıcıları ve modelleri `provider`, `model` ve `small_model` seçenekleri aracılığıyla yapılandırabilirsiniz.
...
codetutor yapılandırmanızda kullanmak istediğiniz temayı `theme` seçeneği aracılığıyla yapılandırabilirsiniz.
...
Agent'ları `~/.config/opencode/agents/` veya `.codetutor/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Varsayılan agent, birincil agent olmalıdır (alt agent değil). Bu, `"build"` veya `"plan"` gibi yerleşik bir agent ya da tanımladığınız bir [özel agent](/docs/agents) olabilir. Belirtilen agent mevcut değilse veya bir alt agent ise, codetutor bir uyarıyla birlikte `"build"`'ye geri döner.

Bu ayar tüm arayüzler için geçerlidir: TUI, CLI (`codetutor run`), masaüstü uygulaması ve GitHub Action.
...
Komutları `~/.config/opencode/commands/` veya `.codetutor/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
codetutor başlatıldığında yeni güncellemeleri otomatik olarak indirecektir. Bunu `autoupdate` seçeneğiyle devre dışı bırakabilirsiniz.
...
Varsayılan olarak, codetutor açık bir onay gerektirmeden **tüm işlemlere izin verir**. `permission` seçeneğini kullanarak bunu değiştirebilirsiniz.
...
Eklenti dosyalarını `.codetutor/plugins/` veya `~/.config/opencode/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
