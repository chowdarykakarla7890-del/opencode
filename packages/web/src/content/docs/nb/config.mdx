...
Du kan konfigurere CodeTutor ved å bruke en JSON konfigurasjonsfil.

---

## Format

CodeTutor støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
2. **Global konfigurasjon** (`~/.config/opencode/codetutor.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`CODETUTOR_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`codetutor.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.codetutor` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`CODETUTOR_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.codetutor` og `~/.config/opencode` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Plasser din globale CodeTutor-konfigurasjon i `~/.config/opencode/codetutor.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `codetutor.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når CodeTutor starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `CODETUTOR_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `CODETUTOR_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.codetutor` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.codetutor`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`codetutor-docs.vercel.app/config.json`**](https://codetutor-docs.vercel.app/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `codetutor serve` og `codetutor web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage CodeTutor-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `codetutor.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i CodeTutor-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver CodeTutor å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/opencode/agents/` eller `.codetutor/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil CodeTutor falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`codetutor run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/opencode/commands/` eller `.codetutor/commands/`. [Les mer her](/docs/commands).
...
CodeTutor vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater CodeTutor alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider CodeTutor med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.codetutor/plugins/` eller `~/.config/opencode/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense CodeTutor til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
