...
Da biste otklonili probleme s CodeTutor, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
CodeTutor pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
CodeTutor Desktop pokreće lokalni CodeTutor server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `CodeTutor` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/opencode/codetutor.jsonc` (ili `~/.config/opencode/codetutor.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/opencode/codetutor.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opencode\codetutor.jsonc`
...
CodeTutor također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga CodeTutor mogao ponovo izgraditi.

1. Potpuno zatvorite CodeTutor Desktop.
...
3. Ponovo pokrenite CodeTutor Desktop.

---

### Rješavanje problema sa vezom na serveru

CodeTutor Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `codetutor.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `CODETUTOR_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `CODETUTOR_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, CodeTutor Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa CodeTutor karakteristikama.

---

### Obavještenja se ne prikazuju

CodeTutor Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za CodeTutor u postavkama vašeg OS-a, i
...
1. Zatvorite CodeTutor Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka CodeTutor Desktop aplikacije):
...
Ako imate problema s CodeTutor:
...
Da saznate kojim modelima imate pristup, pokrenite `codetutor models`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. CodeTutor dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
2. Ponovo pokrenite CodeTutor da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti CodeTutor da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
CodeTutor će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
