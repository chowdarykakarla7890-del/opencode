---
title: Server
description: Komunicirajte s codetutor serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `codetutor serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji codetutor klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `codetutor.local` |
...
Postavite `CODETUTOR_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `codetutor`, ili postavite `CODETUTOR_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `codetutor serve` i za `codetutor web`.
...
Kada pokrenete `codetutor`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite codetutor server za programsku interakciju sa opencode-om.
:::

Ova arhitektura omogucava codetutor podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `codetutor serve` da startate standalone server. Ako je opencode TUI vec pokrenut, `codetutor serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste CodeTutor [IDE](/docs/ide) pluginovi.
...
codetutor server izlaže sljedece API-je.
