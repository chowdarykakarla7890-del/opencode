...
Avec CodeTutor, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, CodeTutor utilise notre propre thème `codetutor`.
...
CodeTutor est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que CodeTutor corresponde à l'apparence de leur terminal
...
CodeTutor prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
3. **Répertoire racine du projet** - Défini dans `<project-root>/.codetutor/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.codetutor/themes/*.json`
...
```bash no-frame
mkdir -p .codetutor/themes
vim .codetutor/themes/my-theme.json
```
...
{
  "$schema": "https://codetutor-docs.vercel.app/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
