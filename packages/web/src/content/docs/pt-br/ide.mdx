---
title: IDE
description: A extensão codetutor para VS Code, Cursor e outros IDEs
---

O codetutor integra-se com VS Code, Cursor ou qualquer IDE que suporte um terminal. Basta executar `codetutor` no terminal para começar.

---

## Uso

- **Lançamento Rápido**: Use `Cmd+Esc` (Mac) ou `Ctrl+Esc` (Windows/Linux) para abrir o codetutor em uma visualização de terminal dividido, ou focar em uma sessão de terminal existente se uma já estiver em execução.
- **Nova Sessão**: Use `Cmd+Shift+Esc` (Mac) ou `Ctrl+Shift+Esc` (Windows/Linux) para iniciar uma nova sessão de terminal opencode, mesmo que uma já esteja aberta. Você também pode clicar no botão codetutor na interface.
...
Para instalar o codetutor no VS Code e forks populares como Cursor, Windsurf, VSCodium:

1. Abra o VS Code
2. Abra o terminal integrado
3. Execute `codetutor` - a extensão será instalada automaticamente
...
- Certifique-se de que você está executando `codetutor` no terminal integrado.
