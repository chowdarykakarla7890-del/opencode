...
codetutor używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.
...
1. Uruchom polecenie `/connect` w TUI, wybierz `OpenCode Zen` i przejdź do [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/zen).
...
Działa jak każdy inny dostawca w codetutor i jest całkowicie opcjonalny w użyciu.

---

## CodeTutor Go

CodeTutor Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół opencode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z opencode.

1. Uruchom polecenie `/connect` w TUI, wybierz `CodeTutor Go` i przejdź do [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/zen).
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `codetutor.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w codetutor nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować codetutor do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod codetutor działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
codetutor używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
...
`codetutor.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację codetutor jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja codetutor będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować codetutor tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować codetutor tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować codetutor tak, aby korzystał z modeli lokalnych poprzez Ollamę.
...
6. **Ważne**: Przed użyciem modeli chmurowych w codetutor musisz pobrać informacje o modelu lokalnie:
...
4. Utwórz lub zaktualizuj plik `codetutor.json` w katalogu projektu:
...
Pola `limit` pozwalają codetutor zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `codetutor auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację codetutor i:
