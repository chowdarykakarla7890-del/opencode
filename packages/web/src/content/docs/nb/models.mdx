...
De fleste populære leverandørene er forhåndslastet som standard. Hvis du har lagt til legitimasjonen for en leverandør gjennom kommandoen `/connect`, vil den være tilgjengelig når du starter CodeTutor.
...
Her er flere modeller som fungerer godt med CodeTutor, uten spesiell rekkefølge. (Dette er ikke en uttømmende liste og er heller ikke nødvendigvis oppdatert):
...
Mange modeller støtter flere varianter med forskjellige konfigurasjoner. codetutor leveres med innebygde standardvarianter for populære leverandører.

### Innebygde varianter

CodeTutor leveres med standardvarianter for mange leverandører:
...
Når CodeTutor starter opp, ser den etter modeller i følgende prioriterte rekkefølge:

1. Kommandolinjeflagget `--model` eller `-m`. Formatet er det samme som i konfigurasjonsfilen: `provider_id/model_id`.

2. Modelllisten i CodeTutor-konfigurasjonen.
