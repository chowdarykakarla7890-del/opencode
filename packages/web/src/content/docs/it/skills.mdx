...
Le skill degli agenti permettono a CodeTutor di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
CodeTutor cerca in queste posizioni:

- Config di progetto: `.codetutor/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, CodeTutor risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.codetutor/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.
...
Crea `.codetutor/skills/git-release/SKILL.md` cosi':
...
CodeTutor elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `codetutor.json`:
...
**Per agenti integrati** (in `codetutor.json`):
