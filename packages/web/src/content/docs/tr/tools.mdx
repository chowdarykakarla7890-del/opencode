...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. codetutor yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
codetutor içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `CODETUTOR_EXPERIMENTAL_LSP_TOOL=true` (veya `CODETUTOR_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca codetutor provider kullanılırken veya `CODETUTOR_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

codetutor başlatırken etkinleştirmek için:
