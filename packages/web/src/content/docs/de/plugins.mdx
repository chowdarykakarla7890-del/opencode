---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um CodeTutor zu erweitern.
---

Mit Plugins können Sie CodeTutor erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von CodeTutor zu ändern.
...
- `.codetutor/plugins/` – Plugins auf Projektebene
...
1. Globale Konfiguration (`~/.config/opencode/codetutor.json`)
2. Projektkonfiguration (`codetutor.json`)
3. Globales Plugin-Verzeichnis (`~/.config/opencode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.codetutor/plugins/`)
...
```json title=".codetutor/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
CodeTutor führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.

```ts title=".codetutor/plugins/my-plugin.ts"
...
```js title=".codetutor/plugins/example.js"
...
- `client`: Ein CodeTutor-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie CodeTutor erweitern können.
...
```js title=".codetutor/plugins/notification.js"
...
:::note
Wenn Sie die CodeTutor-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass CodeTutor `.env`-Dateien liest:

```javascript title=".codetutor/plugins/env-protection.js"
...
```javascript title=".codetutor/plugins/inject-env.js"
...
Plugins können CodeTutor auch benutzerdefinierte Tools hinzufügen:

```ts title=".codetutor/plugins/custom-tools.ts"
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das CodeTutor aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für CodeTutor zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://codetutor-docs.vercel.app/docs/sdk).
...
```ts title=".codetutor/plugins/compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
...
```ts title=".codetutor/plugins/custom-compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
