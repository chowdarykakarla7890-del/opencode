---
title: Serveur
description: Interagissez avec le serveur codetutor via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `codetutor serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client codetutor peut utiliser.
...
```bash
codetutor serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `codetutor.local` |
| `--cors`        | Origines de navigateur supplémentaires à autoriser | `[]`             |

`--cors` peut être transmis plusieurs fois :

```bash
codetutor serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Définissez `CODETUTOR_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `codetutor`, ou définissez `CODETUTOR_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `codetutor serve` et à `codetutor web`.

```bash
CODETUTOR_SERVER_PASSWORD=your-password codetutor serve
```
...
Lorsque vous exécutez `codetutor`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur codetutor pour interagir avec codetutor par programmation.
:::

Cette architecture permet à codetutor de prendre en charge plusieurs clients et vous permet d'interagir avec codetutor par programme.

Vous pouvez exécuter `codetutor serve` pour démarrer un serveur autonome. Si vous avez le
opencode TUI en cours d'exécution, `codetutor serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins CodeTutor [IDE](/docs/ide).
...
Le serveur codetutor expose les API suivantes.
