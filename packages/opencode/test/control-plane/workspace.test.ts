...
const originalEnv = {
  CODETUTOR_AUTH_CONTENT: process.env.CODETUTOR_AUTH_CONTENT,
  CODETUTOR_EXPERIMENTAL_WORKSPACES: process.env.CODETUTOR_EXPERIMENTAL_WORKSPACES,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.CODETUTOR_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.CODETUTOR_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.CODETUTOR_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.CODETUTOR_EXPERIMENTAL_WORKSPACES).toBe("true")
