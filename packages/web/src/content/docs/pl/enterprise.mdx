...
description: Bezpieczne korzystanie z codetutor w Twojej organizacji.
---
...
:::note
codetutor nie przechowuje żadnego kodu ani danych kontekstowych.
:::
...
codetutor jest oprogramowaniem typu open source i nie przechowuje żadnego kodu ani danych kontekstowych, więc programiści mogą po prostu [zacząć](/docs/) i próbować próbną.

---

### Obsługa danych

**codetutor nie dotyczy kodu źródłowego ani danych kontekstowych.** Całe zdarzenie odbywa się lokalnie lub poprzez bezpośrednio wywołane API do twojego dostawcy AI.
...
Jeśli użytkownicy są połączeni z `/share`, połączenie jest połączone z danymi, które zostały przesłane do usług, które są udostępniane przez hosty tych użytkowników w codetutor-docs.vercel.app.
...
Gdy zasięgsz okres próbny i nadzorujesz użytkowanie z codetutor na
...
Możliwości wykorzystania codetutor tak, aby skorzystać z jednego zastosowania dla całej organizacji.
...
Dzięki zastosowaniu konfiguracji codetutor może zostać włączone z dostawcą SSO Twojej organizacji w celu uwierzytelnienia.

Dzięki temu codetutor może uzyskać dane uwierzytelniające dla wewnętrznej bramy AI za pośrednictwem transmisji systemu zarządzania tożsamością.

---

### Wewnętrzna bramka AI

Dzięki zastosowaniu konfiguracji codetutor można również skorzystać z tego, że można korzystać tylko z wewnętrznych bram AI.
...
Po prostu uruchom wersję próbną ze swoim. codetutor urządzenia niezabezpieczone kodu ani danych kontekstowych, co ułatwia uruchomienie pracy.
...
Tak. codetutor nie przechowuje kodu ani danych kontekstowych. Całe zdarzenie odbywa się lokalnie lub poprzez bezpośrednie wywołanie API przez dostawcę AI. Dostęp do konfiguracji i integracji z logowaniem jednokrotnym Twoje dane pozostają bezpieczne w infrastrukturze organizacji.
...
codetutor obsługuje prywatny rejestry npm poprzez natywną usługę plików `.npmrc` Buna. Jeśli Twoja organizacja korzysta z rejestru prywatnego, takiego jak JFrog Artifactory, Nexus lub wylot, przed uruchomieniem codetutor się, że programiści zostali uwierzytelnieni.
...
Spowoduje to do `~/.npmrc` ze szczegółami uwierzytelniania. codetutor zrobi to automatycznie
podniesc do.

:::caution
Przed uruchomieniem codetutor musisz być zalogowany do rejestru prywatnego.
:::
