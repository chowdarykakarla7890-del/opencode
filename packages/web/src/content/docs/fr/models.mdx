...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez CodeTutor.
...
Voici plusieurs modèles qui fonctionnent bien avec CodeTutor, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration CodeTutor.

```json title="codetutor.json" {3}
{
...
```jsonc title="codetutor.jsonc" {7-12,19-24}
{
  "$schema": "https://codetutor-docs.vercel.app/config.json",
  "provider": {
    "openai": {
...
```jsonc title="codetutor.jsonc" {6-21}
{
  "$schema": "https://codetutor-docs.vercel.app/config.json",
  "provider": {
    "opencode": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. CodeTutor est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

CodeTutor est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="codetutor.jsonc" {7-18}
{
  "$schema": "https://codetutor-docs.vercel.app/config.json",
  "provider": {
    "openai": {
...
Lorsque CodeTutor démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration CodeTutor.
