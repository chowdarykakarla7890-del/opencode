---
title: Konfiguration
description: Verwenden Sie die Konfiguration CodeTutor JSON.
---

Sie können CodeTutor mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

CodeTutor unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
2. **Globale Konfiguration** (`~/.config/opencode/codetutor.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`CODETUTOR_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`codetutor.json` im Projekt) – projektspezifische Einstellungen
5. **`.codetutor` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`CODETUTOR_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
:::note
Die Verzeichnisse `.codetutor` und `~/.config/opencode` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Platzieren Sie Ihre globale CodeTutor-Konfiguration in `~/.config/opencode/codetutor.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `codetutor.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.

:::tip
Platzieren Sie die projektspezifische Konfiguration im Stammverzeichnis Ihres Projekts.
:::

Wenn CodeTutor gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
Geben Sie mithilfe der Umgebungsvariablen `CODETUTOR_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `CODETUTOR_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
Umgebungsvariable. Dieses Verzeichnis wird nach Agenten, Befehlen,
Modi und Plugins genau wie das Standardverzeichnis `.codetutor` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.codetutor` geladen, sodass es deren Einstellungen **überschreiben** kann.

---

## Schema

Die Konfigurationsdatei verfügt über ein Schema, das in [**`codetutor-docs.vercel.app/config.json`**](https://codetutor-docs.vercel.app/config.json) definiert ist.
...
Sie können Servereinstellungen für die Befehle `codetutor serve` und `codetutor web` über die Option `server` konfigurieren.
...
- `mdns` – mDNS-Diensterkennung aktivieren. Dadurch können andere Geräte im Netzwerk Ihres CodeTutor-Servers erkannt werden.
- `mdnsDomain` – Benutzerdefinierter Domänenname für den mDNS-Dienst. Standardmäßig ist `codetutor.local`. Nützlich für die Ausführung mehrerer Instanzen im selben Netzwerk.
...
Sie können die Anbieter und Modelle, die Sie in Ihrer CodeTutor-Konfiguration verwenden möchten, über die Optionen `provider`, `model` und `small_model` konfigurieren.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht CodeTutor, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Sie können das Thema, das Sie in Ihrer CodeTutor-Konfiguration verwenden möchten, über die Option `theme` konfigurieren.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/opencode/agents/` oder `.codetutor/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift CodeTutor mit einer Warnung auf `"build"` zurück.

Diese Einstellung gilt für alle Schnittstellen: TUI, CLI (`codetutor run`), Desktop-App und GitHub Aktion.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/opencode/commands/` oder `.codetutor/commands/` definieren. [Mehr erfahren](/docs/commands).
...
CodeTutor lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
CodeTutor erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert CodeTutor mit benutzerdefinierten Tools, Hooks und Integrationen.

Platzieren Sie Ihre Plugin-Dateien in `.codetutor/plugins/` oder `~/.config/opencode/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
...
Dies ist nützlich, wenn Sie CodeTutor darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.
