---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i CodeTutor.
---

import config from "../../../../config.mjs"
export const console = config.console

CodeTutor bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i CodeTutor-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i CodeTutor
konfig.
...
OpenCode Zen er en liste over modeller levert av CodeTutor-teamet som har vært
testet og verifisert for å fungere godt med CodeTutor. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med OpenCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `OpenCode Zen` og gå til [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/zen).
...
Det fungerer som alle andre leverandører i CodeTutor og er helt valgfritt å bruke.

---

## CodeTutor Go

CodeTutor Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av CodeTutor-teamet som har vært
testet og verifisert for å fungere godt med CodeTutor.

1. Kjør kommandoen `/connect` i TUI, velg `CodeTutor Go`, og gå til [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/zen).
...
Slik bruker du Amazon Bedrock med CodeTutor:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører CodeTutor:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `codetutor.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i codetutor støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere codetutor til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at CodeTutor skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom CodeTutor-konfigurasjonen.
...
for å lagre token i CodeTutor auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
CodeTutor bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse CodeTutor
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`codetutor.json` fil. Det anbefales også å deaktivere øktdeling.
...
Tilpass gjennom `codetutor.json`:
...
Slik bruker du GitHub Copilot-abonnementet med CodeTutor:
...
Slik bruker du Google Vertex AI med CodeTutor:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører CodeTutor.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom CodeTutor, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver CodeTutor-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver CodeTutor-samtale oppført som en separat økt.
...
Du kan konfigurere CodeTutor for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere CodeTutor for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere CodeTutor for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for CodeTutor. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med CodeTutor:
...
3. Kopier API-nøkkelen for bruk i CodeTutor.
...
6. **Viktig**: Før du bruker skymodeller i CodeTutor, må du hente modellinformasjonen lokalt:
...
OpenCode Zen er en liste over testede og verifiserte modeller levert av CodeTutor-teamet. [Finn ut mer](/docs/zen).
...
3. Skriv inn CodeTutor API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom CodeTutor-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom CodeTutor-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med CodeTutor:
...
Du kan også tilpasse modeller gjennom CodeTutor-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med CodeTutor. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `codetutor.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar CodeTutor forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `codetutor auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk CodeTutor-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i CodeTutor-konfigurasjonen.
