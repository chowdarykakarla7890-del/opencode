...
```json
{
  "$schema": "https://codetutor-docs.vercel.app/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
...
- The value is a semver range checked against the running CodeTutor version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.codetutor` is absent, no check is performed (backward compatible).
...
- `codetutor plugin <module>` and TUI install both run install → manifest read → config patch.
...
- For local scope, path is `<worktree>/.codetutor` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.codetutor`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.codetutor`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.codetutor` dir or `CODETUTOR_CONFIG_DIR`, CodeTutor installs `@opencode-ai/plugin` into that dir and writes:
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus CodeTutor's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
CodeTutor registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, CodeTutor pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- Local plugins persist installed themes under the local `.codetutor/themes` area near the plugin config source.
...
- `--pure` / `CODETUTOR_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.codetutor/plugins/tui-smoke.tsx`
- Local vim plugin: `.codetutor/plugins/tui-vim.tsx`
- Local smoke config: `.codetutor/tui.json`
- Local smoke theme: `.codetutor/plugins/smoke-theme.json`
