...
エージェントスキルにより、CodeTutor はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
CodeTutor は次の場所を検索します。

- プロジェクト設定: `.codetutor/skills/<name>/SKILL.md`
...
プロジェクトのローカルパスの場合、CodeTutor は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
途中で、一致する `skills/*/SKILL.md` を `.codetutor/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。
...
次のように `.codetutor/skills/git-release/SKILL.md` を作成します。
...
CodeTutor では、`skill` ツールの説明に利用可能なスキルがリストされています。
...
`codetutor.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。
...
**組み込みエージェントの場合** (`codetutor.json` 内):
