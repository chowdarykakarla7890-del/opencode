---
title: Konfiguration
description: Ved at bruge CodeTutor JSON konfig.
---

Du kan konfigurere CodeTutor ved hjælp af en JSON-konfigurationsfil.

---

## Format

CodeTutor understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
2. **Global config** (`~/.config/opencode/codetutor.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`CODETUTOR_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`codetutor.json` i projekt) - projektspecifikke indstillinger
5. **`.codetutor` mapper** - agent, kommandoer, plugins
6. **Inline config** (`CODETUTOR_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.codetutor` og `~/.config/opencode` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Placer din globale CodeTutor-konfiguration i `~/.config/opencode/codetutor.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `codetutor.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når CodeTutor starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `CODETUTOR_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `CODETUTOR_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.codetutor` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.codetutor` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`codetutor-docs.vercel.app/config.json`**](https://codetutor-docs.vercel.app/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `codetutor serve` og `codetutor web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din CodeTutor-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `codetutor.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din CodeTutor-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger CodeTutor at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din CodeTutor-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/opencode/agents/` eller `.codetutor/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil CodeTutor falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`codetutor run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/opencode/commands/` eller `.codetutor/commands/`. [Learn more here](/docs/commands).
...
CodeTutor vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
[Plugins](/docs/plugins) udvide CodeTutor med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.codetutor/plugins/` eller `~/.config/opencode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse CodeTutor til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
