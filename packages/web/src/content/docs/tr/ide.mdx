---
title: IDE
description: VS Code, Cursor ve diğer IDE'ler için codetutor uzantısı
---

opencode, VS Code, Cursor veya bir terminali destekleyen herhangi bir IDE ile entegre olur. Başlamak için terminalde `codetutor` komutunu çalıştırmanız yeterli.
...
- **Yeni Oturum**: Zaten açık olsa bile yeni bir codetutor terminal oturumu başlatmak için `Cmd+Shift+Esc` (Mac) veya `Ctrl+Shift+Esc` (Windows/Linux) kullanın. Ayrıca kullanıcı arayüzündeki codetutor düğmesini de tıklayabilirsiniz.
- **Bağlam Farkındalığı**: Geçerli seçiminizi veya sekmenizi codetutor ile otomatik olarak paylaşın.
...
3. `codetutor` komutunu çalıştırın - uzantı otomatik olarak yüklenir
...
- Entegre terminalde `codetutor` çalıştırdığınızdan emin olun.
