---
title: TUI
description: Utilisation de l’interface utilisateur du terminal CodeTutor.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodeTutor fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de CodeTutor démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le CodeTutor TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à CodeTutor. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez CodeTutor. *Alias* : `/quit`, `/q`
...
Ceci est séparé de `codetutor.json`, qui configure le comportement du serveur/d'exécution.
...
- `leader_timeout` - Contrôle la durée pendant laquelle CodeTutor attend après la leader key. Valeur par défaut : `2000`.
