...
Die meisten gängigen Anbieter sind standardmäßig vorinstalliert. Wenn Sie die Anmeldeinformationen für einen Anbieter über den Befehl `/connect` hinzugefügt haben, sind diese verfügbar, wenn Sie CodeTutor starten.
...
Hier sind mehrere Modelle, die gut mit CodeTutor funktionieren, in keiner bestimmten Reihenfolge. (Dies ist weder eine vollständige noch eine unbedingt aktuelle Liste):
...
Viele Modelle unterstützen mehrere Varianten mit unterschiedlichen Konfigurationen. CodeTutor wird mit integrierten Standardvarianten für beliebte Anbieter ausgeliefert.

### Integrierte Varianten

CodeTutor wird für viele Anbieter mit Standardvarianten ausgeliefert:
...
Wenn CodeTutor startet, sucht es nach Modellen in der folgenden Prioritätsreihenfolge:

1. Das Befehlszeilenflag `--model` oder `-m`. Das Format ist das gleiche wie in der Konfigurationsdatei: `provider_id/model_id`.

2. Die Modellliste in der CodeTutor-Konfiguration.
