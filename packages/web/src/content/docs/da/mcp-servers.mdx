...
Du kan tilføje eksterne værktøjer til CodeTutor ved hjælp af _Model Context Protocol_ eller MCP. CodeTutor understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [CodeTutor Config](https://codetutor-docs.vercel.app/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
CodeTutor håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil CodeTutor:
...
Hvis serveren kræver godkendelse, vil CodeTutor bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `codetutor mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil CodeTutor opbevare tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem CodeTutor-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde CodeTutor til din Sentry-konto.
