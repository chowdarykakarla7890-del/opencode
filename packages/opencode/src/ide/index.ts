...
export function alreadyInstalled() {
  return process.env["CODETUTOR_CALLER"] === "vscode" || process.env["CODETUTOR_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.codetutor"], {
    nothrow: true,
  })
