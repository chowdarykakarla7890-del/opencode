---
title: Proveedores
description: Usando cualquier proveedor LLM en CodeTutor.
---

import config from "../../../../config.mjs"
export const console = config.console

CodeTutor usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración CodeTutor.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu CodeTutor
configuración.
...
OpenCode Zen es una lista de modelos proporcionados por el equipo CodeTutor que han sido
probado y verificado para funcionar bien con CodeTutor. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con OpenCode Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione codetutor y diríjase a [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/auth).
...
Funciona como cualquier otro proveedor en CodeTutor y su uso es completamente opcional.

---

## CodeTutor Go

CodeTutor Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de CodeTutor que han sido
probado y verificado para funcionar bien con CodeTutor.

1. Ejecute el comando `/connect` en TUI, seleccione `CodeTutor Go` y diríjase a [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/zen).
...
Para usar Amazon Bedrock con CodeTutor:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Para una configuración persistente o específica del proyecto, utilice `codetutor.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en CodeTutor no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar codetutor para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que codetutor funcione correctamente.
   :::
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `CodeTutor`, Alcances: `api`
...
CodeTutor utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear CodeTutor
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `codetutor.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `codetutor.json`:
...
Para utilizar Google Vertex AI con CodeTutor:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación CodeTutor como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación CodeTutor enumerada como una sesión separada.
...
Puede configurar codetutor para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar codetutor para usar modelos locales a través de LM Studio.
...
Puedes configurar codetutor para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para CodeTutor. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con CodeTutor:
...
3. Copie la clave API para usarla en CodeTutor.
...
6. **Importante**: Antes de usar modelos en la nube en CodeTutor, debe obtener la información del modelo localmente:
...
OpenCode Zen es una lista de modelos probados y verificados proporcionada por el equipo CodeTutor. [Más información](/docs/zen).
...
3. Ingrese su clave CodeTutor API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con CodeTutor:
...
4. Cree o actualice su archivo `codetutor.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a CodeTutor comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `codetutor auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración codetutor y:
