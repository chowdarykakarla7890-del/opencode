...
Mozete dodati prilagodena uputstva za codetutor tako sto kreirate `AGENTS.md` datoteku. Ovo je slicno pravilima u Cursor-u. Sadrzi uputstva koja se ubacuju u LLM kontekst da prilagode ponasanje za vas projekat.
...
Ovo skenira projekat i njegov sadrzaj, razumije cemu projekat sluzi i generise `AGENTS.md`. Tako codetutor bolje navigira kroz kod.
...
codetutor podrzava citanje `AGENTS.md` datoteke sa vise lokacija. Svaka lokacija ima drugu svrhu.
...
Mozete imati i globalna pravila u `~/.config/opencode/AGENTS.md`. Ona se primjenjuju u svim codetutor sesijama.
...
Za korisnike koji prelaze sa Claude Code, CodeTutor podrzava i Claude konvencije datoteka kao rezervu:
...
Kada se codetutor pokrene, trazi datoteke pravila ovim redoslijedom:
...
Mozete navesti prilagodene datoteke uputstava u `codetutor.json` ili globalnom `~/.config/opencode/codetutor.json`. Tako vi i tim ponovo koristite postojeca pravila bez dupliranja u AGENTS.md.
...
Iako codetutor ne parsira automatski reference datoteka u `AGENTS.md`, slicno ponasanje mozete dobiti na dva nacina:

### Korištenje codetutor.json

Preporuceni pristup je da koristite `instructions` polje u `codetutor.json`:
...
Mozete nauciti codetutor da cita eksterne datoteke tako sto cete dati eksplicitna uputstva u `AGENTS.md`. Evo prakticnog primjera:
...
- Osigurate da codetutor ucitava datoteke samo kad su potrebne za konkretan zadatak

:::tip
Za monorepo projekte ili projekte sa zajednickim standardima, odrzivije je koristiti `codetutor.json` sa glob obrascima (npr. `packages/*/AGENTS.md`) nego rucna uputstva.
:::
