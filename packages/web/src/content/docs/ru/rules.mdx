...
Вы можете указать собственные файлы инструкций в `codetutor.json` или в глобальном `~/.config/opencode/codetutor.json`. Это позволит вам и вашей команде повторно использовать существующие правила вместо того, чтобы дублировать их на AGENTS.md.
...
### Использование codetutor.json

Рекомендуемый подход — использовать поле `instructions` в `codetutor.json`:
...
:::tip
Для монорепозиториев или проектов с общими стандартами использование `codetutor.json` с шаблонами glob (например, `packages/*/AGENTS.md`) более удобно в обслуживании, чем инструкции вручную.
:::
