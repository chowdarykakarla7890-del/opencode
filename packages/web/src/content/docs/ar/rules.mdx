...
للمستخدمين الذين ينتقلون من Claude Code، يدعم CodeTutor اصطلاحات ملفات Claude Code كبدائل احتياطية:
...
يمكنك تحديد ملفات تعليمات مخصصة في `codetutor.json` أو في الملف العام `~/.config/opencode/codetutor.json`. يتيح ذلك لك ولفريقك إعادة استخدام القواعد الموجودة بدلًا من تكرارها داخل AGENTS.md.
...
### استخدام codetutor.json

النهج الموصى به هو استخدام الحقل `instructions` في `codetutor.json`:
...
:::tip
بالنسبة للمستودعات متعددة الحزم (monorepos) أو المشاريع ذات المعايير المشتركة، فإن استخدام `codetutor.json` مع أنماط glob (مثل `packages/*/AGENTS.md`) أسهل للصيانة من التعليمات اليدوية.
:::
