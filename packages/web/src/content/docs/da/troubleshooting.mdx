...
For at fejlfinde problemer med CodeTutor, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
codetutor gemmer sessionsdata og andre applikationsdata på disken på:
...
CodeTutor Desktop kører en lokal CodeTutor-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `CodeTutor`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/opencode/codetutor.jsonc` (eller `~/.config/opencode/codetutor.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/opencode/codetutor.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opencode\codetutor.jsonc`
...
CodeTutor kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så CodeTutor kan genopbygge den.

1. Afslut CodeTutor Desktop helt.
2. Slet cache-mappen:
...
3. Start CodeTutor Desktop på ny.

---

### Løs problemer med serverforbindelse

CodeTutor Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `codetutor.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `CODETUTOR_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `CODETUTOR_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver CodeTutor Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i CodeTutor.

---

### Meddelelser vises ikke

CodeTutor Desktop viser kun systemvarsler når:

- varsler er aktiveret for CodeTutor i dine OS-indstillinger, og
...
1. Afslut CodeTutor Desktop.
2. Find og slet disse filer (de findes i CodeTutor Desktop-appens datamappe):
...
Hvis du oplever problemer med CodeTutor:
...
For at finde ud af hvilke modeller du har adgang til, kør `codetutor models`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. codetutor installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
2. Start codetutor på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge codetutor til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
codetutor vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
