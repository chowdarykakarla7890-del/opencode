...
function configuredWorkspaceID(): WorkspaceV2.ID | undefined {
  return Flag.CODETUTOR_WORKSPACE_ID ? WorkspaceV2.ID.make(Flag.CODETUTOR_WORKSPACE_ID) : undefined
}
...
function defaultDirectory(request: HttpServerRequest.HttpServerRequest, url: URL): string {
  return url.searchParams.get("directory") || request.headers["x-codetutor-directory"] || process.cwd()
}
