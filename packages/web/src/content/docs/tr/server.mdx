---
title: Sunucu
description: codetutor sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`codetutor serve` komutu, codetutor istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `codetutor.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `CODETUTOR_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `codetutor` değeridir; değiştirmek isterseniz `CODETUTOR_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `codetutor serve` hem de `codetutor web` için geçerlidir.
...
`codetutor` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
codetutor ile programatik etkileşim için codetutor sunucusunu kullanın.
:::

Bu mimari, opencode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`codetutor serve` ile bağımsız bir sunucu başlatabilirsiniz. opencode TUI açıksa, `codetutor serve` yeni bir sunucu başlatır.
...
codetutor sunucusu aşağıdaki API'leri sunar.
