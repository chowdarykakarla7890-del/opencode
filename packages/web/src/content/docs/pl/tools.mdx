...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. CodeTutor posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w CodeTutor.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `CODETUTOR_EXPERIMENTAL_LSP_TOOL=true` (lub `CODETUTOR_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy CodeTutor lub gdy zmienna środowiskowa `CODETUTOR_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu CodeTutor:
