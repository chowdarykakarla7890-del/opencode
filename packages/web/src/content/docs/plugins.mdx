---
title: Plugins
description: Write your own plugins to extend CodeTutor.
---

Plugins allow you to extend CodeTutor by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify CodeTutor's default behavior.
...
- `.codetutor/plugins/` - Project-level plugins
...
4. Project plugin directory (`.codetutor/plugins/`)
...
CodeTutor runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
:::note
If you’re using the CodeTutor desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Prevent codetutor from reading `.env` files:
...
The `tool` helper creates a custom tool that codetutor can call. It takes a Zod schema function and returns a tool definition with:

- `description`: What the tool does
- `args`: Zod schema for the tool's arguments
- `execute`: Function that runs when the tool is called

Your custom tools will be available to codetutor alongside built-in tools.
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://codetutor-docs.vercel.app/docs/sdk) for details.
