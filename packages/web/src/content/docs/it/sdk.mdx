...
L'SDK JS/TS di codetutor fornisce un client type-safe per interagire con il server.
Usalo per creare integrazioni e controllare codetutor in modo programmatico.
...
Puoi passare un oggetto di configurazione per personalizzare il comportamento. L'istanza legge comunque `codetutor.json`, ma puoi sovrascrivere o aggiungere configurazione inline:
...
Se hai gia un'istanza di codetutor in esecuzione, puoi creare un client per collegarti:
