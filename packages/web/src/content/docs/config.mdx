---
title: Config
description: Using the CodeTutor JSON config.
---

You can configure CodeTutor using a JSON config file.

---

## Format

CodeTutor supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="codetutor.jsonc"
{
...
2. **Global config** (`~/.config/opencode/codetutor.json`) - user preferences
3. **Custom config** (`CODETUTOR_CONFIG` env var) - custom overrides
4. **Project config** (`codetutor.json` in project) - project-specific settings
5. **`.codetutor` directories** - agents, commands, plugins
6. **Inline config** (`CODETUTOR_CONFIG_CONTENT` env var) - runtime overrides
...
:::note
The `.codetutor` and `~/.config/opencode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Place your global CodeTutor config in `~/.config/opencode/codetutor.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.
...
Add `codetutor.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When CodeTutor starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `CODETUTOR_CONFIG` environment variable.

```bash
export CODETUTOR_CONFIG=/path/to/my/custom-config.json
codetutor run "Hello world"
```
...
Specify a custom config directory using the `CODETUTOR_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.codetutor` directory, and should
follow the same structure.

```bash
export CODETUTOR_CONFIG_DIR=/path/to/my/config-directory
codetutor run "Hello world"
```

The custom directory is loaded after the global config and `.codetutor` directories, so it **can override** their settings.
...
Drop an `codetutor.json` or `codetutor.jsonc` file in the system managed config directory:
...
On macOS, CodeTutor reads managed preferences from the `ai.codetutor.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

CodeTutor checks these paths:

1. `/Library/Managed Preferences/<user>/ai.codetutor.managed.plist`
2. `/Library/Managed Preferences/ai.codetutor.managed.plist`

The plist keys map directly to `codetutor.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.codetutor.managed` PayloadType. The CodeTutor config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.codetutor.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.codetutor.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.codetutor</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`codetutor-docs.vercel.app/config.json`**](https://codetutor-docs.vercel.app/config.json).

TUI config uses [**`codetutor-docs.vercel.app/tui.json`**](https://codetutor-docs.vercel.app/tui.json).
...
Use `CODETUTOR_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `codetutor.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `codetutor serve` and `codetutor web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your CodeTutor server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `codetutor.local`. Useful for running multiple instances on the same network.
...
If not specified, CodeTutor will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your CodeTutor config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, CodeTutor tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny CodeTutor actions on configured resources. Currently, policies can control which providers CodeTutor may use.
...
CodeTutor normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, CodeTutor omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/opencode/agents/` or `.codetutor/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, CodeTutor will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`codetutor run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/opencode/commands/` or `.codetutor/commands/`. [Learn more here](/docs/commands).
...
CodeTutor uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
CodeTutor will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
[Plugins](/docs/plugins) extend CodeTutor with custom tools, hooks, and integrations.

Place plugin files in `.codetutor/plugins/` or `~/.config/opencode/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict CodeTutor to only use specific providers rather than disabling them one by one.
