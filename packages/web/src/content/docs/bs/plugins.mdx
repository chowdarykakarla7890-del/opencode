---
title: Dodaci
description: Napišite vlastite dodatke za proširenje CodeTutor.
---

Dodaci vam omogućavaju da proširite CodeTutor spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja CodeTutor.
...
- `.codetutor/plugins/` - Dodaci na nivou projekta
...
1. Globalna konfiguracija (`~/.config/opencode/codetutor.json`)
2. Konfiguracija projekta (`codetutor.json`)
3. Globalni direktorij dodataka (`~/.config/opencode/plugins/`)
4. Direktorij dodataka projekta (`.codetutor/plugins/`)
...
CodeTutor pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje CodeTutor.
...
Spriječite codetutor da čita `.env` fajlove:
...
Pomoćnik `tool` kreira prilagođeni alat koji codetutor može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za codetutor zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://codetutor-docs.vercel.app/docs/sdk) za detalje.
