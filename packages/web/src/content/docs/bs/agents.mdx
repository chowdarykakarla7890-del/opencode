...
Postoje dvije vrste agenata u CodeTutor: primarni agenti i podagenti.
...
CodeTutor dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
CodeTutor dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

CodeTutor dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u CodeTutor-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `codetutor.json`:
...
Ako temperatura nije navedena, CodeTutor koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu CodeTutor konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj CodeTutor konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [OpenCode Zen](/docs/zen), koristili biste `opencode/gpt-5.1-codex` za GPT 5.1 Codex.
...
```json title="codetutor.json" {3-6,9-12}
{
  "$schema": "https://codetutor-docs.vercel.app/config.json",
  "tools": {
    "write": true,
...
```json title="codetutor.json" {3-5,8-10}
{
  "$schema": "https://codetutor-docs.vercel.app/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Pokrenite `codetutor models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/chowdarykakarla7890-del/opencode).
:::
