...
description: Usa la configurazione JSON di CodeTutor.
---

Puoi configurare CodeTutor usando un file di configurazione JSON.

---

## Formato

CodeTutor supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
2. **Config globale** (`~/.config/opencode/codetutor.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `CODETUTOR_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`codetutor.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.codetutor`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `CODETUTOR_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.codetutor` e `~/.config/opencode` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Metti la configurazione globale di CodeTutor in `~/.config/opencode/codetutor.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Aggiungi `codetutor.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando CodeTutor si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `CODETUTOR_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `CODETUTOR_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.codetutor` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.codetutor`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`codetutor-docs.vercel.app/config.json`**](https://codetutor-docs.vercel.app/config.json).

La configurazione TUI usa [**`codetutor-docs.vercel.app/tui.json`**](https://codetutor-docs.vercel.app/tui.json).
...
Usa `CODETUTOR_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `codetutor.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `codetutor serve` e `codetutor web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server CodeTutor.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `codetutor.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in CodeTutor tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, CodeTutor prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/opencode/agents/` o `.codetutor/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, CodeTutor fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`codetutor run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/opencode/commands/` o `.codetutor/commands/`. [Scopri di piu](/docs/commands).
...
CodeTutor scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
[I plugin](/docs/plugins) estendono CodeTutor con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.codetutor/plugins/` o `~/.config/opencode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare CodeTutor a usare solo alcuni provider invece di disabilitarli uno per uno.
