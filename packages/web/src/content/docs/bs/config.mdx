---
title: Konfiguracija
description: Korištenje CodeTutor JSON konfiguracije.
---

Možete konfigurirati CodeTutor koristeći JSON konfiguracijski fajl.

---

## Format

CodeTutor podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
2. **Globalna konfiguracija** (`~/.config/opencode/codetutor.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`CODETUTOR_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`codetutor.json` u projektu) - postavke specifične za projekat
5. **`.codetutor` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`CODETUTOR_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.codetutor` i `~/.config/opencode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Postavite svoju globalnu CodeTutor konfiguraciju u `~/.config/opencode/codetutor.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Dodajte `codetutor.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se CodeTutor pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `CODETUTOR_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `CODETUTOR_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.codetutor` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.codetutor`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`codetutor-docs.vercel.app/config.json`**](https://codetutor-docs.vercel.app/config.json).

TUI konfiguracija koristi [**`codetutor-docs.vercel.app/tui.json`**](https://codetutor-docs.vercel.app/tui.json).
...
Koristite `CODETUTOR_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `codetutor.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `codetutor serve` i `codetutor web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš CodeTutor server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `codetutor.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj CodeTutor konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, CodeTutor pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/opencode/agents/` ili `.codetutor/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, CodeTutor će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`codetutor run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/opencode/commands/` ili `.codetutor/commands/`. [Saznajte više ovdje](/docs/commands).
...
CodeTutor će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, CodeTutor **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju CodeTutor sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.codetutor/plugins/` ili `~/.config/opencode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite CodeTutor da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
