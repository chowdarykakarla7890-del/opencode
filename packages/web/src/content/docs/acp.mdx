---
title: ACP Support
description: Use CodeTutor in any ACP-compatible editor.
---

CodeTutor supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use CodeTutor via ACP, configure your editor to run the `codetutor acp` command.

The command starts CodeTutor as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
To open it, use the new 'CodeTutor' agent in the AI Chat agent selector.
...
To use CodeTutor as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use CodeTutor as the ACP agent for chat.
...
CodeTutor works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your CodeTutor config
