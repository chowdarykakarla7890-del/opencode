...
Para depurar problemas con CodeTutor, comience verificando los registros y los datos locales que almacena en el disco.
...
codetutor almacena datos de sesión y otros datos de aplicaciones en el disco en:
...
CodeTutor Desktop ejecuta un servidor CodeTutor local (el sidecar `opencode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `CodeTutor` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/opencode/codetutor.jsonc` (o `~/.config/opencode/codetutor.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/opencode/codetutor.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\opencode\codetutor.jsonc`
...
CodeTutor también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:
...
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.codetutor/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que CodeTutor pueda reconstruirlo.

1. Salga de CodeTutor Desktop por completo.
...
3. Reinicie el escritorio CodeTutor.

---

### Solucionar problemas de conexión del servidor

CodeTutor Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `codetutor.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `CODETUTOR_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `CODETUTOR_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio CodeTutor requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de CodeTutor.

---

### Notificaciones que no se muestran

CodeTutor Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para CodeTutor en la configuración de su sistema operativo, y
...
1. Salga del escritorio CodeTutor.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio CodeTutor):
...
Si tiene problemas con CodeTutor:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/chowdarykakarla7890-del/opencode/issues**](https://github.com/chowdarykakarla7890-del/opencode/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**codetutor-docs.vercel.app/discord**](https://codetutor-docs.vercel.app/discord)
...
3. Asegúrese de tener la última versión con `codetutor upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `codetutor models`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. codetutor instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
2. Reinicie codetutor para reinstalar los paquetes del proveedor más recientes.

Esto obligará a codetutor a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
codetutor detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
