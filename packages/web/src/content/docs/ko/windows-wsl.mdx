...
:::caution
`--hostname 0.0.0.0`을 사용할 때는 `CODETUTOR_SERVER_PASSWORD`를 설정해 서버를 보호하세요.
...
1. **PowerShell 대신 WSL terminal에서 `codetutor web` 실행하기**
...
WSL에서 `codetutor web`를 실행하면 올바른 파일 시스템 접근과 terminal 통합을 유지하면서도 Windows 브라우저에서 바로 사용할 수 있습니다.
