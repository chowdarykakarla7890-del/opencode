...
Dzięki codetutor możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie codetutor używa naszego własnego motywu `codetutor`.
...
codetutor ma kilka wbudowanych motywów.
...
- Chcą, aby CodeTutor pasował do wyglądu ich terminala
...
codetutor obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.codetutor/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.codetutor/themes/*.json`
