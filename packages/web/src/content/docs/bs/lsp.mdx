---
title: LSP serveri
description: CodeTutor se integriše sa vašim LSP serverima.
---

CodeTutor se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

CodeTutor dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `CODETUTOR_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i codetutor otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj codetutor konfiguraciji.
