---
title: Server
description: Interact with codetutor server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `codetutor serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an codetutor client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `codetutor.local` |
...
Set `CODETUTOR_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `codetutor`, or set `CODETUTOR_SERVER_USERNAME` to override it. This applies to both `codetutor serve` and `codetutor web`.
...
When you run `codetutor` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the codetutor server to interact with codetutor programmatically.
:::

This architecture lets codetutor support multiple clients and allows you to interact with codetutor programmatically.

You can run `codetutor serve` to start a standalone server. If you have the
opencode TUI running, `codetutor serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the CodeTutor [IDE](/docs/ide) plugins.
...
The codetutor server exposes the following APIs.
