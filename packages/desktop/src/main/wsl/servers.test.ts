...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "CodeTutor update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating CodeTutor", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.codetutor/bin/opencode",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, codetutorChecks: {} })
})
...
test("ignores stale background CodeTutor checks after removing a WSL server", async () => {
  persistedServers = []
  ...
  expect(controller.getState().codetutorChecks).toEqual({})
})

test("ignores stale startup CodeTutor checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking CodeTutor", async () => {
  persistedServers = []
  ...
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveOpencode: async (distro) => {
      ...
      return "/home/me/.codetutor/bin/opencode"
    },
  })
  ...
  expect(Object.keys(controller.getState().codetutorChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check CodeTutor in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const opencode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().codetutorChecks)).toEqual(["Debian"])
})
