...
Możesz dodać dodatkowe informacje do codetutor za pomocą _Model Context Protocol_, w skrócie MCP. codetutor obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [opencode Config](https://codetutor-docs.vercel.app/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
codetutor automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, opencode:
...
Jeśli serwer wymaga uwierzytelnienia, codetutor poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `codetutor mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji codetutor bezpieczne przechowa tokeny w `~/.local/share/opencode/mcp-auth.json`.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć codetutor z kontem Sentry.
