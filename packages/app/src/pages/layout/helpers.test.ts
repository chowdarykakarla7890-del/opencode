...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("codetutor://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("codetutor://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("codetutor://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("codetutor://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("codetutor://open-project")).toBeUndefined()
    expect(parseDeepLink("codetutor://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "codetutor://open-project?directory=/a",
      "codetutor://other?directory=/b",
      "codetutor://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("codetutor://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("codetutor://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("codetutor://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("codetutor://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "codetutor://new-session?directory=/a",
      "codetutor://open-project?directory=/b",
      "codetutor://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __CODETUTOR__: {
        deepLinks: ["codetutor://open-project?directory=/a"],
      },
    } as unknown as Window & { __CODETUTOR__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["codetutor://open-project?directory=/a"])
