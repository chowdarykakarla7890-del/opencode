...
Možete dodati vanjske alate u CodeTutor koristeći _Model Context Protocol_, ili MCP. CodeTutor podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [CodeTutor Config](https://codetutor-docs.vercel.app/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
CodeTutor automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, CodeTutor će:
...
Ako server zahtijeva autentifikaciju, CodeTutor će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `codetutor mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, CodeTutor sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u CodeTutor, zajedno s ugrađenim alatima. Možete njima upravljati kroz CodeTutor konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete CodeTutor sa vašim Sentry nalogom.
