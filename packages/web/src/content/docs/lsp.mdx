---
title: LSP Servers
description: CodeTutor integrates with your LSP servers.
---

CodeTutor can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

CodeTutor comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `CODETUTOR_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and codetutor opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your codetutor config.
...
```json title="codetutor.json" {5-13}
{
  "$schema": "https://codetutor-docs.vercel.app/config.json",
  "lsp": {
    "custom-lsp": {
