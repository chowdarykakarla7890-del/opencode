...
Aby uzyskać dostęp do codetutor w swoim terminalu, będziesz potrzebować:
...
Najłatwiej zainstalować codetutor za pomocą skryptu instalacyjnego.
...
Obecnie pracujemy nad instalacją codetutor w systemie Windows za pomocą Bun.

Możesz także pobrać plik binarny z [Releases](https://github.com/chowdarykakarla7890-del/opencode/releases).

---

## Konfiguracja

Dzięki codetutor możesz korzystać z dowolnego dostawcy LLM, konfigurując jego klucze API.
...
1. Uruchom polecenie `/connect` w TUI, wybierz codetutor i przejdź do [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/auth).
...
Następnie zainicjuj codetutor dla projektu, uruchamiając odpowiednie polecenie.

```bash frame="none"
/init
```

Spowoduje to, że codetutor przeanalizuje Twój projekt i utworzy plik `AGENTS.md` w katalogu głównym projektu.

:::tip
Powinieneś zacommitować plik `AGENTS.md` swojego projektu w Git.
:::

Pomaga to codetutor zrozumieć strukturę projektu i stosowane wzorce kodowania.
...
Możesz poprosić codetutor o wyjaśnienie bazy kodu.
...
Możesz poprosić codetutor o dodanie nowej funkcji do Twojego projektu. Najpierw zalecamy poproszenie o przygotowanie planu.

1. **Stwórz Plan**

   codetutor ma _Plan Mode_, który umożliwia wyłączenie zmian i
   ...
   Wersja codetutor jest bardzo inteligentna, aby zrozumieć, czego chcesz. To pomaga
   rozmawiaj z nim tak, jakbyś rozmawiał z młodszym programistą w swoim zespole.

   :::tip
   Podaj codetutor kontekst, aby zrozumiał, czego chcesz.
   :::
...
2. **Udoskonalanie planu**
   ...
   codetutor może zeskanować obrazy, które mu przekażesz i zrozumieć je. Możesz to zrobić, przeciągając i upuszczając obraz do terminala.
...
Aby wprowadzić kolejną zmianę, możesz poprosić codetutor lub zrobić to bezpośrednio bez konieczności wcześniejszego przeglądania planu.
...
Musisz upewnić się, że dostarczyłeś odpowiednie pliki, aby codetutor mógł wprowadzić zmiany.

---

### Cofnij zmiany

Załóżmy, że prosisz codetutor o wprowadzenie zmian.
...
codetutor cofnie teraz zmiany i wyświetli wiadomość ponownie.

```txt frame="none" "@packages/functions/src/api/index.ts"
Can you refactor the function in @packages/functions/src/api/index.ts?
```

Możesz edytować monit i poprosić codetutor o ponowną próbę.
...
Rozmowy z codetutor można [udostępniać swojemu zespołowi](/docs/share).
...
Oto [przykładowa rozmowa](https://codetutor-docs.vercel.app/s/4XP1fce5) z opencode.
