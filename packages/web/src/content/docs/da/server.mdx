...
Kommandoen `codetutor serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en opencode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `codetutor.local` |
...
Indstil `CODETUTOR_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `codetutor`, eller indstil `CODETUTOR_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `codetutor serve` og `codetutor web`.
...
Når du kører `codetutor` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug opencode-serveren til at interagere med codetutor programmatisk.
:::

Denne arkitektur lader codetutor understøtte flere klienter og lader dig interagere med codetutor programmatisk.

Du kan køre `codetutor serve` for at starte en selvstændig server. Hvis du har
opencode TUI kørende, vil `codetutor serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af CodeTutor [IDE](/docs/ide) plugins.
...
CodeTutor-serveren viser følgende API'er.
