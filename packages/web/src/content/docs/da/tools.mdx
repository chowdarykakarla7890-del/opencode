...
Her er alle de innebygde verktøyene som er tilgængelige i CodeTutor.
...
:::note
Dette verktøyet er bare tilgængelig når `CODETUTOR_EXPERIMENTAL_LSP_TOOL=true` (eller `CODETUTOR_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger CodeTutor-leverandøren eller når miljøvariabelen `CODETUTOR_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
