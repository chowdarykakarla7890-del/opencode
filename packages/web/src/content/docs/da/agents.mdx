...
Der er to typer agenter i CodeTutor; primære agenter og subagenter.
...
CodeTutor leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
CodeTutor leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

CodeTutor leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i CodeTutor's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `codetutor.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger CodeTutor modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale CodeTutor-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din CodeTutor-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [OpenCode Zen](/docs/zen), vil du bruge `opencode/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `codetutor models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/chowdarykakarla7890-del/opencode).
:::
