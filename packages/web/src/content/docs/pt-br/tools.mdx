...
As ferramentas permitem que o LLM execute ações em sua base de código. O codetutor vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
:::note
Esta ferramenta está disponível apenas quando `CODETUTOR_EXPERIMENTAL_LSP_TOOL=true` (ou `CODETUTOR_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor codetutor ou quando a variável de ambiente `CODETUTOR_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).
