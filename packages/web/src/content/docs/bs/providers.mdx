---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u CodeTutor.
---

import config from "../../../../config.mjs"
export const console = config.console

CodeTutor koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj CodeTutor konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem CodeTutor
config.
...
OpenCode Zen je lista modela koje je obezbedio CodeTutor tim koji su bili
testirano i potvrđeno da dobro radi sa CodeTutor. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OpenCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite codetutor i idite na [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/auth).
...
Radi kao i svaki drugi provajder u CodeTutor i potpuno je opcionalan za korištenje.

---

## CodeTutor Go

CodeTutor Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža CodeTutor tim i koji su testirani i verificirani da dobro rade s CodeTutor-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `CodeTutor Go` i idite na [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/zen).
...
Da biste koristili Amazon Bedrock s CodeTutor:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `codetutor.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u CodeTutor.
:::
...
Možete konfigurirati codetutor za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi codetutor ispravno radio.
   :::
...
Također možete dodati modele kroz svoju codetutor konfiguraciju.
...
da pohrani token u codetutor auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
CodeTutor koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate CodeTutor
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`codetutor.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `codetutor.json`:
...
Za korištenje Google Vertex AI s CodeTutor:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki CodeTutor razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki CodeTutor razgovor naveden kao zasebna sesija.
...
Možete konfigurirati codetutor za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati codetutor za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati codetutor za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za CodeTutor. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s CodeTutor:
...
3. Kopirajte API ključ za korištenje u CodeTutor.
...
6. **Važno**: Prije upotrebe modela oblaka u CodeTutor, morate lokalno povući informacije o modelu:
...
OpenCode Zen je lista testiranih i verifikovanih modela koju je obezbedio CodeTutor tim. [Saznajte više](/docs/zen).
...
3. Unesite svoj CodeTutor API ključ.
...
Također možete dodati dodatne modele putem vaše codetutor konfiguracije.
...
5. Također ih možete prilagoditi putem vaše codetutor konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju codetutor konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
4. Kreirajte ili ažurirajte svoju `codetutor.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju CodeTutor da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `codetutor auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite CodeTutor konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj codetutor konfiguraciji.
