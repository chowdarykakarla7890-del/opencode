...
_Model Bağlam Protokolü_ veya MCP kullanarak opencode'a harici araçlar ekleyebilirsiniz. codetutor hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [opencode Config](https://codetutor-docs.vercel.app/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Sunucu kimlik doğrulama gerektiriyorsa opencode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `codetutor mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
MCP'leriniz yerleşik araçların yanı sıra opencode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi codetutor yapılandırması aracılığıyla yönetebilirsiniz.
