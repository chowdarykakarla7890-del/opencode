...
Você pode adicionar ferramentas externas ao codetutor usando o _Model Context Protocol_, ou MCP. O codetutor suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do opencode](https://codetutor-docs.vercel.app/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
O codetutor lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o codetutor irá:
...
Se o servidor requer autenticação, o codetutor solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `codetutor mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o codetutor armazenará os tokens de forma segura em `~/.local/share/opencode/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no opencode, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do codetutor como qualquer outra ferramenta.
