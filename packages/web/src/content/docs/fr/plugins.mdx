---
title: Plugins
description: Écrivez vos propres plugins pour étendre CodeTutor.
---

Les plugins vous permettent d'étendre CodeTutor en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de CodeTutor.
...
- `.codetutor/plugins/` - Plugins au niveau du projet
...
1. Configuration globale (`~/.config/opencode/codetutor.json`)
2. Configuration du projet (`codetutor.json`)
3. Répertoire global des plugins (`~/.config/opencode/plugins/`)
4. Répertoire des plugins du projet (`.codetutor/plugins/`)
...
CodeTutor exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK codetutor pour interagir avec l'IA.
...
:::note
Si vous utilisez l'application de bureau CodeTutor, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez codetutor de lire les fichiers `.env` :
...
L'assistant `tool` crée un outil personnalisé que codetutor peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour codetutor aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://codetutor-docs.vercel.app/docs/sdk) pour plus de détails.
