...
Plugins permitem que você estenda o codetutor conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do opencode.
...
- `.codetutor/plugins/` - Plugins em nível de projeto
...
1. Configuração global (`~/.config/opencode/codetutor.json`)
2. Configuração do projeto (`codetutor.json`)
3. Diretório de plugins global (`~/.config/opencode/plugins/`)
4. Diretório de plugins do projeto (`.codetutor/plugins/`)
...
O codetutor executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do codetutor para interagir com a AI.
...
Impeça o codetutor de ler arquivos `.env`:
...
O helper `tool` cria uma ferramenta personalizada que o codetutor pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o codetutor junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://codetutor-docs.vercel.app/docs/sdk) para detalhes.
