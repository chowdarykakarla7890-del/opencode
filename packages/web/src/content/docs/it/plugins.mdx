---
title: Plugin
description: Scrivi plugin per estendere CodeTutor.
---

I plugin ti permettono di estendere CodeTutor agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di CodeTutor.
...
- `.codetutor/plugins/` - plugin a livello progetto
...
1. Config globale (`~/.config/opencode/codetutor.json`)
2. Config di progetto (`codetutor.json`)
3. Directory plugin globale (`~/.config/opencode/plugins/`)
4. Directory plugin di progetto (`.codetutor/plugins/`)
...
CodeTutor esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di codetutor per interagire con l'AI.
...
:::note
Se usi l'app desktop di CodeTutor, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a codetutor di leggere i file `.env`:
...
L'helper `tool` crea uno strumento personalizzato che codetutor può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in codetutor insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://codetutor-docs.vercel.app/docs/sdk) per i dettagli.
