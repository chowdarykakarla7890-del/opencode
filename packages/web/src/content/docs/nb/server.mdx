---
title: Tjener
description: Samhandle med CodeTutor-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `codetutor serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en CodeTutor-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `codetutor.local` |
...
Sett `CODETUTOR_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `codetutor` som standard, eller sett `CODETUTOR_SERVER_USERNAME` for å overstyre det. Dette gjelder både `codetutor serve` og `codetutor web`.
...
Når du kjører `codetutor` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk CodeTutor-serveren til å samhandle med CodeTutor programmatisk.
:::

Denne arkitekturen lar CodeTutor støtte flere klienter og lar deg samhandle med CodeTutor programmatisk.

Du kan kjøre `codetutor serve` for å starte en frittstående server. Hvis du har
CodeTutor TUI kjører, vil `codetutor serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av CodeTutor [IDE](/docs/ide) plugins.
