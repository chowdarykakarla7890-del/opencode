...
description: Lag verktøy som LLM kan kalle inn CodeTutor.
---

Egendefinerte verktøy er funksjoner du oppretter som LLM kan kalle under samtaler. De fungerer sammen med CodeTutor sine [innebygde verktøy](/docs/tools) som `read`, `write` og `bash`.
...
- Lokalt ved å plassere dem i `.codetutor/tools/`-katalogen til prosjektet ditt.
...
```ts title=".codetutor/tools/bash.ts"
import { tool } from "@opencode-ai/plugin"
