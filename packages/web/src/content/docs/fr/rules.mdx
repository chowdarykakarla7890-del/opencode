...
Vous pouvez fournir des instructions personnalisées à codetutor en créant un fichier `AGENTS.md`. Ceci est similaire aux règles du curseur. Il contient des instructions qui seront incluses dans le contexte du LLM pour personnaliser son comportement pour votre projet spécifique.
...
codetutor prend également en charge la lecture du fichier `AGENTS.md` à partir de plusieurs emplacements. Et cela répond à des objectifs différents.
...
Pour les utilisateurs migrant depuis Claude Code, CodeTutor prend en charge les conventions de fichiers de Claude Code comme solutions de secours :
...
Lorsque codetutor démarre, il recherche les fichiers de règles dans cet ordre :
...
Vous pouvez spécifier des fichiers d'instructions personnalisés dans votre `codetutor.json` ou dans le `~/.config/opencode/codetutor.json` global. Cela vous permet, à vous et à votre équipe, de réutiliser les règles existantes plutôt que d'avoir à les dupliquer dans AGENTS.md.
...
Bien que codetutor n'analyse pas automatiquement les références de fichiers dans `AGENTS.md`, vous pouvez obtenir des fonctionnalités similaires de deux manières :

### Utilisation de codetutor.json

L'approche recommandée consiste à utiliser le champ `instructions` dans `codetutor.json` :
...
- Assurez-vous que codetutor charge les fichiers uniquement lorsque cela est nécessaire pour une tâche spécifique.

:::tip
Pour les monorepos ou les projets avec des normes partagées, l'utilisation de `codetutor.json` avec des modèles globaux (comme `packages/*/AGENTS.md`) est plus maintenable que les instructions manuelles.
:::
