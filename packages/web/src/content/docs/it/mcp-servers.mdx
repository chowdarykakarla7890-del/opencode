...
Puoi aggiungere strumenti esterni a CodeTutor usando il _Model Context Protocol_, o MCP. CodeTutor supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [CodeTutor Config](https://codetutor-docs.vercel.app/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
CodeTutor gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, CodeTutor:
...
Se il server richiede autenticazione, CodeTutor ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `codetutor mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, CodeTutor memorizzera' i token in modo sicuro in `~/.local/share/opencode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in CodeTutor insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione CodeTutor come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare CodeTutor al tuo account Sentry.
