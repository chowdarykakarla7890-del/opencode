...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".codetutor/glossary/zh-cn.md"
  if (locale === "zht") return ".codetutor/glossary/zh-tw.md"
  return `.codetutor/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("CodeTutor did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("CodeTutor returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("CodeTutor session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel CodeTutor runs for 'all' (default: 4)
      --model <provider/id>  CodeTutor model (default: opencode/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running CodeTutor
...
`)
  ...
  if (failed.length) console.error(`\nCodeTutor failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.CODETUTOR_DISABLE_PROJECT_CONFIG = "1"
  env.CODETUTOR_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
...
function isolatedEnvironment() {
  ...
  delete env.CODETUTOR_CONFIG
  delete env.CODETUTOR_CONFIG_DIR
  delete env.CODETUTOR_CONFIG_CONTENT
  delete env.CODETUTOR_PERMISSION
  delete env.CODETUTOR_AUTO_SHARE
  return env
}
