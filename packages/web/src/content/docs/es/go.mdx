...
CodeTutor Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

Go funciona como cualquier otro proveedor en CodeTutor. Te suscribes a CodeTutor Go y
...
:::tip
Probamos un grupo selecto de modelos y proveedores que funcionan bien con CodeTutor.
:::
...
CodeTutor Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

CodeTutor Go funciona como cualquier otro proveedor en CodeTutor.

1. Inicias sesión en **<a href={console}>OpenCode Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `CodeTutor Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a CodeTutor Go.
:::
...
CodeTutor Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de CodeTutor
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con CodeTutor.
