...
export const PrCommand = effectCmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run CodeTutor",
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    UI.println()
    UI.println("Starting CodeTutor...")
    UI.println()

    const code = yield* Effect.promise(
      () =>
        Process.spawn(["codetutor"], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
    ...
    if (code !== 0) return yield* Effect.die(new Error(`CodeTutor exited with code ${code}`))
  }),
})
