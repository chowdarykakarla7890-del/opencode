...
2. Sağlayıcıyı codetutor yapılandırmanızda yapılandırın.
...
OpenCode Zen, codetutor ekibi tarafından sağlanan modellerin bir listesidir.
codetutor ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz OpenCode Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, opencode'u seçin ve [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/auth)'ye gidin.
...
CodeTutor Go, codetutor ile iyi çalıştığı test edilmiş ve doğrulanmış, codetutor ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `CodeTutor Go`'yu seçin ve [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/zen) adresine gidin.
...
Amazon Bedrock'u codetutor ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Projeye özel veya kalıcı yapılandırma için `codetutor.json` kullanın:
...
`codetutor.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`codetutor.json` üzerinden özelleştirin:
...
Google Vertex AI'yi codetutor ile kullanmak için:
...
Helicone'un codetutor aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her codetutor görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her codetutor görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
:::tip
Ollama kendisini codetutor için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/opencode)'a bakın.
:::
...
Ollama Cloud'u codetutor ile kullanmak için:
...
OpenCode Zen, codetutor ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   codetutor yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca codetutor yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri codetutor yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı codetutor ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `codetutor.json` dosyanızı oluşturun veya güncelleyin:
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `codetutor auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için codetutor yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, codetutor yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
