...
1. Запустите команду `/connect` в TUI, выберите `OpenCode Zen` и перейдите по адресу [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/zen).
...
CodeTutor Go — это недорогой план подписки, обеспечивающий надежный доступ к популярным открытым моделям кодирования, предоставляемым командой opencode, которые были
протестированы и проверены на хорошую работу с opencode.

1. Запустите команду `/connect` в TUI, выберите `CodeTutor Go` и перейдите по адресу [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/zen).
...
2. **Настройте аутентификацию** одним из следующих способов:
   ...
   Для конкретной или постоянной конфигурации проекта используйте `codetutor.json`:
...
2. Выберите метод аутентификации:
   ...
   3. Имя: `CodeTutor`, Области применения: `api`
...
`codetutor.json` файл. Также рекомендуется отключить совместное использование сеансов.
...
Настройте через `codetutor.json`:
...
4. Создайте или обновите файл `codetutor.json` в каталоге вашего проекта:
...
1. **Проверьте настройку аутентификации**: запустите `codetutor auth list`, чтобы проверить, верны ли учетные данные.
