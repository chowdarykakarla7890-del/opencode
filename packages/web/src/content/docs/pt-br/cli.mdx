...
Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o codetutor programaticamente.
...
A CLI do codetutor também possui os seguintes comandos.
...
Anexe um terminal a um servidor backend do codetutor já em execução, iniciado via comandos `serve` ou `web`.

```bash
codetutor attach [url]
```

Isso permite usar o TUI com um backend codetutor remoto. Por exemplo:
...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `CODETUTOR_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `CODETUTOR_SERVER_USERNAME` ou `codetutor`) |
...
O opencode é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `codetutor auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/opencode/auth.json`.
...
Execute o codetutor em modo não interativo passando um prompt diretamente.
...
```bash "codetutor run"
codetutor run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `codetutor serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
codetutor run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor codetutor em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `CODETUTOR_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `CODETUTOR_SERVER_USERNAME` ou `codetutor`) |
...
Inicie um servidor codetutor sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
codetutor serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do codetutor sem a interface TUI. Defina `CODETUTOR_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `codetutor`).
...
Inicie um servidor codetutor sem cabeça com uma interface web.

```bash
codetutor web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o codetutor através de uma interface web. Defina `CODETUTOR_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `codetutor`).
...
Desinstale o codetutor e remova todos os arquivos relacionados.
...
Atualiza o codetutor para a versão mais recente ou uma versão específica.
...
A CLI do codetutor aceita as seguintes flags globais.
...
O codetutor pode ser configurado usando variáveis de ambiente.

| Variável                              | Tipo    | Descrição                                                             |
| ------------------------------------- | ------- | --------------------------------------------------------------------- |
| `CODETUTOR_AUTO_SHARE`                 | boolean | Compartilhar sessões automaticamente                                  |
| `CODETUTOR_GIT_BASH_PATH`              | string  | Caminho para o executável do Git Bash no Windows                      |
| `CODETUTOR_CONFIG`                     | string  | Caminho para o arquivo de configuração                                |
| `CODETUTOR_CONFIG_DIR`                 | string  | Caminho para o diretório de configuração                              |
| `CODETUTOR_CONFIG_CONTENT`             | string  | Conteúdo de configuração json inline                                  |
| `CODETUTOR_DISABLE_AUTOUPDATE`         | boolean | Desabilitar verificações de atualização automática                    |
| `CODETUTOR_DISABLE_PRUNE`              | boolean | Desabilitar a poda de dados antigos                                   |
| `CODETUTOR_DISABLE_TERMINAL_TITLE`     | boolean | Desabilitar atualizações automáticas do título do terminal            |
| `CODETUTOR_PERMISSION`                 | string  | Configuração de permissões json inline                                |
| `CODETUTOR_DISABLE_DEFAULT_PLUGINS`    | boolean | Desabilitar plugins padrão                                            |
| `CODETUTOR_DISABLE_LSP_DOWNLOAD`       | boolean | Desabilitar downloads automáticos do servidor LSP                     |
| `CODETUTOR_ENABLE_EXPERIMENTAL_MODELS` | boolean | Habilitar modelos experimentais                                       |
| `CODETUTOR_DISABLE_AUTOCOMPACT`        | boolean | Desabilitar compactação automática de contexto                        |
| `CODETUTOR_DISABLE_CLAUDE_CODE`        | boolean | Desabilitar leitura de `.claude` (prompt + habilidades)               |
| `CODETUTOR_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Desabilitar leitura de `~/.claude/CLAUDE.md`                          |
| `CODETUTOR_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Desabilitar carregamento de `.claude/skills`                          |
| `CODETUTOR_DISABLE_MODELS_FETCH`       | boolean | Desabilitar busca de modelos de fontes remotas                        |
| `CODETUTOR_FAKE_VCS`                   | string  | Provedor VCS falso para fins de teste                                 |
| `CODETUTOR_CLIENT`                     | string  | Identificador do cliente (padrão é `cli`)                             |
| `CODETUTOR_ENABLE_EXA`                 | boolean | Habilitar ferramentas de busca web Exa                                |
| `CODETUTOR_SERVER_PASSWORD`            | string  | Habilitar autenticação básica para `serve`/`web`                      |
| `CODETUTOR_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `codetutor`) |
| `CODETUTOR_MODELS_URL`                 | string  | URL personalizada para buscar configuração de modelos                 |
...
| Variável                                        | Tipo    | Descrição                                                 |
| ----------------------------------------------- | ------- | --------------------------------------------------------- |
| `CODETUTOR_EXPERIMENTAL`                         | boolean | Habilitar recursos experimentais sob o sinalizador global |
| `CODETUTOR_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Habilitar descoberta de ícones                            |
| `CODETUTOR_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Desabilitar cópia ao selecionar no TUI                    |
| `CODETUTOR_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Tempo limite padrão para comandos bash em ms              |
| `CODETUTOR_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Máximo de tokens de saída para respostas LLM              |
| `CODETUTOR_EXPERIMENTAL_FILEWATCHER`             | boolean | Habilitar monitoramento de arquivos para todo o diretório |
| `CODETUTOR_EXPERIMENTAL_OXFMT`                   | boolean | Habilitar formatador oxfmt                                |
| `CODETUTOR_EXPERIMENTAL_LSP_TOOL`                | boolean | Habilitar ferramenta LSP experimental                     |
| `CODETUTOR_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Desabilitar monitoramento de arquivos                     |
| `CODETUTOR_EXPERIMENTAL_EXA`                     | boolean | Habilitar recursos experimentais do Exa                   |
| `CODETUTOR_EXPERIMENTAL_LSP_TY`                  | boolean | Habilitar TY LSP para arquivos python                     |
| `CODETUTOR_EXPERIMENTAL_PLAN_MODE`               | boolean | Habilitar modo de plano                                   |
| `CODETUTOR_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Habilitar tarefas de subagentes em segundo plano          |
| `CODETUTOR_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Habilitar sistema de eventos experimental                 |
| `CODETUTOR_EXPERIMENTAL_NATIVE_LLM`              | boolean | Habilitar caminho nativo de requisição LLM                |
| `CODETUTOR_EXPERIMENTAL_PARALLEL`                | boolean | Habilitar execução paralela de busca web                  |
| `CODETUTOR_EXPERIMENTAL_SCOUT`                   | boolean | Habilitar subagente Scout                                 |
| `CODETUTOR_EXPERIMENTAL_WORKSPACES`              | boolean | Habilitar suporte a espaços de trabalho                   |
