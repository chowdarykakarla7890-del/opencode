...
يشغّل الأمر `codetutor serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل opencode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `codetutor.local` |
...
عيّن `CODETUTOR_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `codetutor`، أو عيّن `CODETUTOR_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `codetutor serve` و `codetutor web`.
...
عند تشغيل `codetutor` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
...
يمكنك تشغيل `codetutor serve` لبدء خادم مستقل. إذا كانت واجهة opencode في terminal (TUI)
قيد التشغيل، فسيبدأ `codetutor serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات CodeTutor لـ [IDE](/docs/ide).
