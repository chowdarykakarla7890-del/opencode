...
`,
  ...
  const pure = process.env.CODETUTOR_PURE
  const meta = process.env.CODETUTOR_PLUGIN_META_FILE
  process.env.CODETUTOR_PURE = "1"
  process.env.CODETUTOR_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.CODETUTOR_PURE
    } else {
      process.env.CODETUTOR_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.CODETUTOR_PLUGIN_META_FILE
    } else {
      process.env.CODETUTOR_PLUGIN_META_FILE = meta
    }
  }
})
