...
Puede agregar herramientas externas a CodeTutor utilizando el _Model Context Protocol_ o MCP. CodeTutor admite servidores locales y remotos.
...
Puede definir servidores MCP en su [CodeTutor Config](https://codetutor-docs.vercel.app/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
CodeTutor maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, CodeTutor:
...
Si el servidor requiere autenticación, CodeTutor le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `codetutor mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, CodeTutor almacenará los tokens de forma segura en `~/.local/share/opencode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en CodeTutor, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración CodeTutor como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar CodeTutor a su cuenta Sentry.
