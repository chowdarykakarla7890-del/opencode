...
You can add external tools to CodeTutor using the _Model Context Protocol_, or MCP. CodeTutor supports both local and remote servers.
...
You can define MCP servers in your [CodeTutor Config](https://codetutor-docs.vercel.app/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
CodeTutor automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, CodeTutor will:
...
If the server requires authentication, CodeTutor will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `codetutor mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, CodeTutor will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in CodeTutor, alongside built-in tools. So you can manage them through the CodeTutor config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect CodeTutor to your Sentry account.
