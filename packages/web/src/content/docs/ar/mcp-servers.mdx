...
يمكنك إضافة أدوات خارجية إلى CodeTutor باستخدام _Model Context Protocol_، أو MCP. يدعم CodeTutor خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات CodeTutor](https://codetutor-docs.vercel.app/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يتولى CodeTutor تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم CodeTutor بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك CodeTutor المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `codetutor mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن CodeTutor الرموز بشكل آمن في `~/.local/share/opencode/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل CodeTutor إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات CodeTutor مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط CodeTutor بحساب Sentry الخاص بك.
