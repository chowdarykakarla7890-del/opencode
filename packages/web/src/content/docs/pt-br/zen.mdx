---
title: Zen
description: Lista selecionada de modelos fornecidos pelo CodeTutor.
---
...
O OpenCode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do CodeTutor.

O Zen funciona como qualquer outro provedor no CodeTutor. Você faz login no OpenCode Zen e recebe sua chave de API. É **completamente opcional** e você não precisa usá-lo para usar o CodeTutor.
...
O OpenCode Zen funciona como qualquer outro provedor no CodeTutor.
...
O [model id](/docs/config/#models) na sua configuração do CodeTutor usa o formato `opencode/<model-id>`. Por exemplo, para GPT 5.5, você usaria `opencode/gpt-5.5` na sua configuração.
...
Você pode notar [modelos de baixo custo](/docs/config/#models), como Haiku, Nano ou Flash, no seu histórico de uso. O CodeTutor usa esses modelos para gerar títulos de sessões.
...
- DeepSeek V4 Flash Free está disponível no CodeTutor por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- MiMo-V2.5 Free está disponível no CodeTutor por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Hy3 Free está disponível no CodeTutor por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Laguna S 2.1 Free está disponível no CodeTutor por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Nemotron 3 Ultra Free está disponível no CodeTutor por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Nemotron 3.5 Lightning Free está disponível no CodeTutor por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Big Pickle é um modelo stealth que está gratuito no CodeTutor por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
...
4. Não ter **lock-in**, permitindo que você o use com qualquer outro agente de codificação. E sempre permitir que você use qualquer outro provedor com o CodeTutor também.
