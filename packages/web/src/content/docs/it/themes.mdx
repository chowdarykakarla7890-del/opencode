...
Con CodeTutor puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, CodeTutor usa il tema `codetutor`.
...
CodeTutor include diversi temi integrati.
...
- Vuole che CodeTutor corrisponda all'aspetto del terminale
...
CodeTutor supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
3. **Directory root del progetto** - in `<project-root>/.codetutor/themes/*.json`
4. **Directory di lavoro corrente** - in `./.codetutor/themes/*.json`
