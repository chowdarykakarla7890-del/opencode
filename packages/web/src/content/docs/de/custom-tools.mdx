...
description: Erstellen Sie Tools, die der LLM in CodeTutor aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von CodeTutor wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.codetutor/tools/` Ihres Projekts platzieren.
...
```ts title=".codetutor/tools/database.ts" {1}
...
```ts title=".codetutor/tools/math.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".codetutor/tools/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".codetutor/tools/add.py"
import sys
...
```ts title=".codetutor/tools/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".codetutor/tools/add.py")
