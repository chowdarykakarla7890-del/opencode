...
As habilidades do agente permitem que o codetutor descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O codetutor pesquisa nesses locais:

- Configuração do projeto: `.codetutor/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o codetutor sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.codetutor/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.
...
Crie `.codetutor/skills/git-release/SKILL.md` assim:
...
O codetutor lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `codetutor.json`:
...
**Para agentes embutidos** (em `codetutor.json`):
