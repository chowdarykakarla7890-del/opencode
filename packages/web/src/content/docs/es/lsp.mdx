...
description: CodeTutor se integra con sus servidores LSP.
---

CodeTutor puede integrarse con servidores Language Server Protocol (LSP) para usar diagnósticos como feedback para el agente.

---

## Incorporados

CodeTutor viene con varios servidores LSP integrados para idiomas populares:
...
:::note
Puede deshabilitar las descargas automáticas del servidor LSP configurando la variable de entorno `CODETUTOR_DISABLE_LSP_DOWNLOAD` en `true`.
:::
...
Cuando LSP está habilitado y codetutor abre un archivo:
