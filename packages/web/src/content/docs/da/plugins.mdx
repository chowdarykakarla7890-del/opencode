---
title: Plugins
description: Skriv dine egne plugins for at udvide CodeTutor.
---

Plugins giver dig mulighed for at udvide CodeTutor ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre CodeTutors standardadfærd.
...
- `.codetutor/plugins/` - Plugins på projektniveau
...
1. Global konfiguration (`~/.config/opencode/codetutor.json`)
2. Projektkonfiguration (`codetutor.json`)
3. Global plugin-mappe (`~/.config/opencode/plugins/`)
4. Projekt plugin bibliotek (`.codetutor/plugins/`)
...
CodeTutor kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
:::note
Hvis du bruger CodeTutor desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå codetutor i at læse `.env` filer:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som codetutor kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for codetutor sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://codetutor-docs.vercel.app/docs/sdk) for detaljer.
