...
Las herramientas permiten que LLM realice acciones en su código base. CodeTutor viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en CodeTutor.
...
:::note
Esta herramienta solo está disponible cuando `CODETUTOR_EXPERIMENTAL_LSP_TOOL=true` (o `CODETUTOR_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor CodeTutor o cuando la variable de entorno `CODETUTOR_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar CodeTutor:

```bash
CODETUTOR_ENABLE_EXA=1 opencode
```
