---
title: Empresa
description: Usando o codetutor com segurança em sua organização.
---
...
:::note
O codetutor não armazena nenhum de seus códigos ou dados de contexto.
:::
...
**O codetutor não armazena seu código ou dados de contexto.** Todo o processamento acontece localmente ou através de chamadas diretas de API para seu provedor de IA.

Isso significa que, enquanto você estiver usando um provedor em quem confia, ou um gateway de IA interno, você pode usar o codetutor com segurança.
...
Se um usuário habilitar o recurso `/share`, a conversa e os dados associados a ela são enviados para o serviço que usamos para hospedar essas páginas de compartilhamento em codetutor-docs.vercel.app.
...
Uma vez que você tenha concluído seu teste e esteja pronto para usar o codetutor em sua organização, você pode **<a href={email}>entrar em contato conosco</a>** para discutir preços e opções de implementação.

---

### Configuração Central

Podemos configurar o codetutor para usar uma única configuração central para toda a sua organização.
...
Através da configuração central, o codetutor pode se integrar ao provedor de SSO de sua organização para autenticação.

Isso permite que o codetutor obtenha credenciais para seu gateway de IA interno através do seu sistema de gerenciamento de identidade existente.

---

### Gateway de IA Interno

Com a configuração central, o codetutor também pode ser configurado para usar apenas seu gateway de IA interno.
...
Sim. O codetutor não armazena seu código ou dados de contexto. Todo o processamento acontece localmente ou através de chamadas diretas de API para seu provedor de IA. Com a configuração central e a integração SSO, seus dados permanecem seguros dentro da infraestrutura de sua organização.
...
O codetutor suporta registros npm privados através do suporte nativo do arquivo `.npmrc` do Bun. Se sua organização usa um registro privado, como JFrog Artifactory, Nexus ou similar, certifique-se de que os desenvolvedores estejam autenticados antes de executar o opencode.
...
Isso cria `~/.npmrc` com os detalhes de autenticação. O codetutor irá automaticamente
capturar isso.
...
Os desenvolvedores devem estar logados no registro privado antes de executar o codetutor para garantir que os pacotes possam ser instalados a partir do seu registro empresarial.
