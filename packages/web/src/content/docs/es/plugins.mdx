---
title: Complementos
description: Escriba sus propios complementos para extender CodeTutor.
---

Los complementos le permiten extender CodeTutor al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de CodeTutor.
...
- `.codetutor/plugins/` - Complementos a nivel de proyecto
...
1. Configuración global (`~/.config/opencode/codetutor.json`)
2. Configuración del proyecto (`codetutor.json`)
3. Directorio global de complementos (`~/.config/opencode/plugins/`)
4. Directorio de complementos del proyecto (`.codetutor/plugins/`)
...
CodeTutor ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK codetutor para interactuar con la IA.
...
:::note
Si está utilizando la aplicación de escritorio CodeTutor, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que codetutor lea archivos `.env`:
...
El ayudante `tool` crea una herramienta personalizada a la que codetutor puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para codetutor junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://codetutor-docs.vercel.app/docs/sdk) para obtener más detalles.
