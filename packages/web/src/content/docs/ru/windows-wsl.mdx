...
:::caution
При использовании `--hostname 0.0.0.0` задайте `CODETUTOR_SERVER_PASSWORD`, чтобы защитить сервер.
...
1. **Запускайте `codetutor web` в терминале WSL**, а не в PowerShell:
...
Запуск `codetutor web` из WSL обеспечивает корректный доступ к файловой системе и интеграцию с терминалом, при этом интерфейс остается доступным из браузера Windows.
