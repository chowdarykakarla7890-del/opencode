---
title: Server LSP
description: CodeTutor si integra con i tuoi server LSP.
---

CodeTutor può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

CodeTutor include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `CODETUTOR_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e codetutor apre un file:
