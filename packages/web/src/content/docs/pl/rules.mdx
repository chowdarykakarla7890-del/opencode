...
codetutor obsługuje także odczytywanie pliku `AGENTS.md` z wielu lokalizacji. A to służy różnym celom.
...
W przypadku użytkowników migrujących z Claude Code codetutor obsługuje konwencje plików Claude Code jako rozwiązania awaryjne:
...
Po uruchomieniu codetutor szuka plików reguł w następującej kolejności:
...
Możesz określić niestandardowe pliki instrukcji w swoim `codetutor.json` lub globalnym `~/.config/opencode/codetutor.json`. Dzięki temu Ty i Twój zespół możecie ponownie wykorzystać istniejące reguły, zamiast duplikować je w pliku AGENTS.md.
...
Chociaż codetutor nie analizuje automatycznie odniesień do plików w `AGENTS.md`, możesz osiągnąć podobną funkcjonalność na dwa sposoby:

### Używanie codetutor.json

Zalecanym podejściem jest użycie pola `instructions` w `codetutor.json`:
...
Możesz nauczyć codetutor czytać pliki zewnętrzne, podając wyraźne instrukcje w swoim `AGENTS.md`. Oto praktyczny przykład:
...
:::tip
W przypadku monorepo lub projektów ze wspólnymi standardami użycie `codetutor.json` ze wzorami globalnymi (takimi jak `packages/*/AGENTS.md`) jest łatwiejsze w utrzymaniu niż instrukcje ręczne.
:::
