...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier CodeTutor launch", () => {
    expect(hasExistingAppState([file("codetutor.settings")])).toBe(true)
    expect(hasExistingAppState([file("codetutor.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("codetutor")])).toBe(true)
  })
})
