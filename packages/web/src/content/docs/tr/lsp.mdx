...
description: CodeTutor, LSP sunucularınızla bütünleşir.
---

CodeTutor, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

CodeTutor, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`CODETUTOR_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve codetutor bir dosyayı açtığında:
...
LSP sunucularını, codetutor yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
