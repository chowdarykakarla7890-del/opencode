...
Você pode configurar o codetutor usando um arquivo de configuração JSON.

---

## Formato

O codetutor suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
2. **Configuração global** (`~/.config/opencode/codetutor.json`) - preferências do usuário
3. **Configuração personalizada** (`CODETUTOR_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`codetutor.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.codetutor`** - agentes, comandos, plugins
6. **Configuração inline** (`CODETUTOR_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.codetutor` e `~/.config/opencode` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
Coloque sua configuração global do codetutor em `~/.config/opencode/codetutor.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.
...
Adicione `codetutor.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `CODETUTOR_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `CODETUTOR_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.codetutor`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.codetutor`, então ele **pode substituir** suas configurações.

---

## Esquema

O esquema de configuração do servidor/tempo de execução é definido em [**`codetutor-docs.vercel.app/config.json`**](https://codetutor-docs.vercel.app/config.json).

A configuração do TUI usa [**`codetutor-docs.vercel.app/tui.json`**](https://codetutor-docs.vercel.app/tui.json).
...
Use `CODETUTOR_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `codetutor.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `codetutor serve` e `codetutor web` através da opção `server`.
...
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `codetutor.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do codetutor através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o codetutor tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/opencode/agents/` ou `.codetutor/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o codetutor recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`codetutor run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/opencode/commands/` ou `.codetutor/commands/`. [Saiba mais aqui](/docs/commands).
...
O codetutor fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
[Plugins](/docs/plugins) estendem o codetutor com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.codetutor/plugins/` ou `~/.config/opencode/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o codetutor para usar apenas provedores específicos, em vez de desabilitá-los um a um.
