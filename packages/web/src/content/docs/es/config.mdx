---
title: Configuración
description: Usando la configuración CodeTutor JSON.
---

Puede configurar CodeTutor usando un archivo de configuración JSON.

---

## Formato

CodeTutor admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
2. **Configuración global** (`~/.config/opencode/codetutor.json`) - preferencias del usuario
3. **Configuración personalizada** (`CODETUTOR_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`codetutor.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.codetutor`** - agentes, comandos, complementos
6. **Configuración en línea** (`CODETUTOR_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
...
:::note
Los directorios `.codetutor` y `~/.config/opencode` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Coloque su configuración global CodeTutor en `~/.config/opencode/codetutor.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `codetutor.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.

:::tip
Coloque la configuración específica del proyecto en la raíz de su proyecto.
:::

Cuando se inicia CodeTutor, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
Especifique una ruta de archivo de configuración personalizada utilizando la variable de entorno `CODETUTOR_CONFIG`.
...
Especifique un directorio de configuración personalizado usando `CODETUTOR_CONFIG_DIR`
variable de entorno. En este directorio se buscarán agentes, comandos,
modos y complementos como el directorio estándar `.codetutor`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.codetutor`, por lo que **puede anular** su configuración.

---

## Esquema

El archivo de configuración tiene un esquema definido en [**`codetutor-docs.vercel.app/config.json`**](https://codetutor-docs.vercel.app/config.json).
...
Puede configurar los ajustes del servidor para los comandos `codetutor serve` y `codetutor web` a través de la opción `server`.
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor CodeTutor.
- `mdnsDomain` - Nombre de dominio personalizado para el servicio mDNS. El valor predeterminado es `codetutor.local`. Útil para ejecutar múltiples instancias en la misma red.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración CodeTutor a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, CodeTutor intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración CodeTutor a través de la opción `theme`.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/opencode/agents/` o `.codetutor/agents/`. [Más información aquí](/docs/agents).
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, CodeTutor recurrirá a `"build"` con una advertencia.

Esta configuración se aplica en todas las interfaces: TUI, CLI (`codetutor run`), aplicación de escritorio y GitHub Acción.
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/opencode/commands/` o `.codetutor/commands/`. [Más información aquí](/docs/commands).
...
CodeTutor descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
Los [complementos](/docs/plugins) amplían CodeTutor con herramientas, enlaces e integraciones personalizados.

Coloque los archivos de complemento en `.codetutor/plugins/` o `~/.config/opencode/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
...
Esto es útil cuando desea restringir CodeTutor para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
