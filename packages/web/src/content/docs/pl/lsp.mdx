---
title: Serwery LSP
description: CodeTutor integruje się z serwerami LSP.
---

CodeTutor może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

CodeTutor posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `CODETUTOR_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i CodeTutor otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji CodeTutor.
