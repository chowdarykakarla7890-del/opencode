...
Wenn CodeTutor Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
codetutor speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
CodeTutor Desktop startet im Hintergrund einen lokalen CodeTutor-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `CodeTutor`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/opencode/codetutor.jsonc` (oder `~/.config/opencode/codetutor.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/opencode/codetutor.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opencode\codetutor.jsonc`
...
CodeTutor kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit CodeTutor ihn neu aufbauen kann.

1. Beende CodeTutor Desktop komplett.
...
3. Starte CodeTutor Desktop neu.

---

### Server-Verbindungsprobleme beheben

CodeTutor Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `codetutor.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `CODETUTOR_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `CODETUTOR_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt CodeTutor Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit CodeTutor-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

CodeTutor Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer CodeTutor in den OS-Einstellungen aktiviert sind, und
...
1. Beende CodeTutor Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von CodeTutor Desktop):
...
Wenn du Probleme mit CodeTutor hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `codetutor models` aus.
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. codetutor installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
2. Starte codetutor neu, um die neuesten Provider-Pakete zu installieren
...
codetutor erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
