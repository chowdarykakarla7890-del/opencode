...
Move the canonical CodeTutor terminal application from
...
The SDK is the TUI's CodeTutor boundary. Missing backend data or operations must
...
- CodeTutor domain data displayed by the TUI
...
route, and feature-plugin code no longer reads CodeTutor globals or process state;
...
Replace process-global CodeTutor state with resolved TUI inputs.
...
adapters rather than CodeTutor domain implementations.

Make the SDK the only CodeTutor domain boundary used by the TUI.
...
- Domain-facing TUI code imports CodeTutor data and operations only from
  `@opencode-ai/sdk`.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for CodeTutor domain state.
