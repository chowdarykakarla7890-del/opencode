---
title: GitLab
description: Use o codetutor em problemas e solicitações de mesclagem do GitLab.
---

O codetutor se integra ao seu fluxo de trabalho do GitLab através do seu pipeline CI/CD do GitLab ou com o GitLab Duo.

Em ambos os casos, o codetutor será executado em seus runners do GitLab.

---

## GitLab CI

O codetutor funciona em um pipeline regular do GitLab. Você pode integrá-lo a um pipeline como um [componente CI](https://docs.gitlab.com/ee/ci/components/)
...
- **Use configuração personalizada por trabalho**: Configure o codetutor com um diretório de configuração personalizado, por exemplo `./config/#custom-directory` para habilitar ou desabilitar funcionalidades por invocação do opencode.
- **Configuração mínima**: O componente CI configura o codetutor em segundo plano, você só precisa criar a configuração do codetutor e o prompt inicial.
...
1. Armazene seu JSON de autenticação do codetutor como variáveis de ambiente do tipo File em **Configurações** > **CI/CD** > **Variáveis**. Certifique-se de marcá-las como "Mascaradas e ocultas".
   ...
   include:
     - component: $CI_SERVER_FQDN/nagyv/gitlab-opencode/opencode@2
       inputs:
         ...
         auth_json: $CODETUTOR_AUTH_JSON # O nome da variável para seu JSON de autenticação do CodeTutor
...
O codetutor se integra ao seu fluxo de trabalho do GitLab.
Mencione `@opencode` em um comentário, e o codetutor executará tarefas dentro do seu pipeline CI do GitLab.
...
- **Corrigir e implementar**: Peça ao codetutor para corrigir um problema ou implementar uma funcionalidade.
...
6.  Crie um arquivo de configuração de fluxo, aqui está um exemplo:
    ...
    commands:
      ...
      - echo $GITLAB_HOST
      - echo "Criando configuração de auth do CodeTutor"
...
Aqui estão alguns exemplos de como você pode usar o codetutor no GitLab.
...
- **Explicar um problema**
  ...
  O codetutor lerá o problema e responderá com uma explicação clara.
...
- **Corrigir um problema**
  ...
  O codetutor criará um novo branch, implementará as alterações e abrirá uma solicitação de mesclagem com as alterações.
...
- **Revisar solicitações de mesclagem**
  ...
  O codetutor revisará a solicitação de mesclagem e fornecerá feedback.
