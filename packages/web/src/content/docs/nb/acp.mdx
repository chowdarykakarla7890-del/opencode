---
title: ACP-støtte
description: Bruk codetutor i en hvilken som helst ACP-kompatibel editor.
---

CodeTutor støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke CodeTutor via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `codetutor acp`.

Kommandoen starter CodeTutor som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'CodeTutor'-agenten i AI Chat-agentvelgeren.
...
For å bruke CodeTutor som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke CodeTutor som ACP-agent for chat.
...
CodeTutor fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i CodeTutor-konfigurasjonen
