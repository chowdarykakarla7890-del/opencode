---
title: Servidor
description: Interactuar con el servidor codetutor a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `codetutor serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente codetutor puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `codetutor.local` |
...
Configure `CODETUTOR_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `codetutor`, o configure `CODETUTOR_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `codetutor serve` como a `codetutor web`.
...
Cuando ejecuta `codetutor`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor codetutor para interactuar con codetutor mediante programación.
:::

Esta arquitectura permite que codetutor admita múltiples clientes y le permite interactuar con codetutor mediante programación.

Puede ejecutar `codetutor serve` para iniciar un servidor independiente. Si tienes el
opencode TUI ejecutándose, `codetutor serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos CodeTutor [IDE](/docs/ide).
...
El servidor codetutor expone las siguientes API.
