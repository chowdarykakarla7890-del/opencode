...
const original = {
  ...
  envPassword: process.env.CODETUTOR_SERVER_PASSWORD,
  envUsername: process.env.CODETUTOR_SERVER_USERNAME,
}
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.CODETUTOR_SERVER_PASSWORD
  else process.env.CODETUTOR_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.CODETUTOR_SERVER_USERNAME
  else process.env.CODETUTOR_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.CODETUTOR_SERVER_PASSWORD = auth.password
  Flag.CODETUTOR_SERVER_USERNAME = auth.username
  process.env.CODETUTOR_SERVER_PASSWORD = auth.password
  process.env.CODETUTOR_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.CODETUTOR_SERVER_USERNAME = auth.username
  delete process.env.CODETUTOR_SERVER_PASSWORD
  process.env.CODETUTOR_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-codetutor-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-codetutor-ticket": "1" }),
  ...
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-codetutor-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    await using tmp = await tmpdir({
      init: async (directory) => {
        ...
        await Bun.write(
          path.join(directory, "codetutor.json"),
          JSON.stringify({ formatter: false, lsp: false, plugin: [pathToFileURL(plugin).href] }),
        )
        return { initialized, completed }
      },
    })
    const previous = process.env.CODETUTOR_DISABLE_DEFAULT_PLUGINS
    process.env.CODETUTOR_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.CODETUTOR_DISABLE_DEFAULT_PLUGINS
      else process.env.CODETUTOR_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-codetutor-ticket": "1" },
      })
