...
CodeTutor gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre CodeTutor starter TUI for gjeldende katalog.
...
Når du bruker CodeTutor TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til CodeTutor. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt CodeTutor. _Aliaser_: `/quit`, `/q`
...
Dette er atskilt fra `codetutor.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge CodeTutor venter etter leader key. Standard er `2000`.
