...
const TEST_ONBOARDING = process.env.CODETUTOR_TEST_ONBOARDING === "1"
const SIDECAR_VERSION = process.env.CODETUTOR_SIDECAR_V2 === "1" ? "v2" : "v1"
...
const main = Effect.gen(function* () {
  ...
  process.env.CODETUTOR_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "com.codetutor.desktop.dev"
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "CodeTutor Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("codetutor://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("codetutor")
  registerRendererProtocol()
  setDockIcon()
  ...
  const loadingTask = yield* Effect.gen(function* () {
    ...
    const port = yield* Effect.gen(function* () {
      const fromEnv = process.env.CODETUTOR_PORT
      if (fromEnv) {
