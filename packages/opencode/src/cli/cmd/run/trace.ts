...
//
// Enable with CODETUTOR_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/codetutor/log/direct/<timestamp>-<pid>.jsonl. Also writes
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.CODETUTOR_DIRECT_TRACE) {
    state = false
    return undefined
  }
