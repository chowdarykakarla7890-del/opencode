---
title: GitHub
description: Użyj codetutor w problemach z GitHubem i zastosujch ściągnięcia.
---

codetutor integruje się z przepływem pracy w GitHub. Wspomnij o `/opencode` lub `/oc` w swoim komentarzu, a codetutor wykonaj zadania w ramach modułu uruchamiającego GitHub Actions.

---

## Cechy

- **Problem związany z segregacją**: Poproś codetutor o szczegółowe wyjaśnienie problemu i wyjaśnienie go.
- **Napraw i zaimplementuj**: Poproś codetutor o naprawienie problemu lub zaimplementowanie funkcji. Będzie dostępny w następnym oddziale i wysyłać PR ze stosowaniem dodatku.
- **Bezpieczny**: codetutor działa w modułach sprzętowych GitHuba.
...
- `token`: opcjonalny token dostępu GitHub podstawowe operacje, takie jak tworzenie komentarzy, zatwierdzanie zmian i otwieranie zastosowania ściągnięcia. Domyślnie codetutor używa tokena dostępu do instalacji z aplikacji opencode GitHub, więc zatwierdzenia, komentarze i zasady ściągnięcia widoczne jako źródło z aplikacji.
...
codetutor może zostać wywołany przez zdarzenie GitHub:

| Typ zdarzenia                 | Wywołane przez                                   | Szczegóły                                                                                                                                             |
| ----------------------------- | ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `issue_comment`               | Skomentuj problem lub PR                         | Wspomnij o `/opencode` lub `/oc` w swoim komentarzu. codetutor odczytuje kontekst i może być częścią składową, otwieraną przez PR lub odpowiedzialną.  |
...
| `issues`                      | Wydanie otwarte lub edytowane                    | Automatycznie wyzwalaj codetutor po utworzeniu lub zmodyfikowaniu problemów. Wymaga wejścia `prompt`.                                                  |
...
| `schedule`                    | Harmonogram oparty na Cron                       | Uruchom codetutor zgodnie z harmonogramem. Wymagane wejście `prompt`. Dane wejściowe trafiają do dzienników i trafiań PR (nie ma problemu z recenzją). |
| `workflow_dispatch`           | Ręczny wyłącznik z interfejsu użytkownika GitHub | Uruchom codetutor na karcie Akcje. Wymagane wejście `prompt`. Dane wejściowe trafiają do dzienników i odbiorców PR.                                    |

### Przykład harmonogramu

Uruchamiaj codetutor zgodnie z harmonogramem, aby wykonać zautomatyzowane zadania:
...
W przypadku zdarzenia `pull_request`, jeśli nie podano `prompt`, codetutor użytkownika przeglądającego ściągnięcie.
...
Zastąp domyślne monit, aby zastosować zachowanie codetutor do twojego własnego pracy.
...
Oto kilka możliwości wykorzystania codetutor w GitHubie.
...
- **Wyjaśnij problem**
  ...
  codetutor przeczytaj całość, zawierając dodatek z komentarzami i odpowiedzią z jasnym wyjaśnieniem.
...
- **Napraw problem**
  ...
  A codetutor utworzy nową podstawę, wdroży zmiany i zastąpi PR ze zmiany.
...
- **Przegląd zmiany PR i wprowadzenie zmiany**
  ...
  codetutor zaimplementuje uruchomioną zmianę i zatwierdzi ją do tego samego PR.

- **Przejrzyj konkretne linie kodu**

  Zostaw komentarz bezpośrednio w wierszu kodu w dodatku „Pliki” PR. codetutor automatyczne wykrywanie pliku, numery linii i kontekst różnicowy, aby sprawdzić odpowiedzi.
  ...
  Komentując określone linie, codetutor otrzymuje:
