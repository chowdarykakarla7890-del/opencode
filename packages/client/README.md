...
Private generation target for clients derived directly from CodeTutor's authoritative Effect `HttpApi`.
...
```ts
import { AbsolutePath, Location, CodeTutor, Prompt } from "@opencode-ai/client/effect"

const client = yield * CodeTutor.make({ baseUrl: "https://opencode.example" })
yield *
