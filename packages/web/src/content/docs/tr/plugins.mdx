---
title: Eklentiler
description: CodeTutor'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek CodeTutor'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya CodeTutor'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.codetutor/plugins/` - Proje düzeyinde eklentiler
...
1. Global config (`~/.config/opencode/codetutor.json`)
2. Project config (`codetutor.json`)
3. Global eklenti dizini (`~/.config/opencode/plugins/`)
4. Proje eklenti dizini (`.codetutor/plugins/`)
...
CodeTutor bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir CodeTutor SDK istemcisi.
...
CodeTutor'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
CodeTutor masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
CodeTutor'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca CodeTutor'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, CodeTutor'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra CodeTutor'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://codetutor-docs.vercel.app/docs/sdk) bakın.
