...
通过 CodeTutor，您可以从多个内置主题中进行选择，使用能自动适配终端主题的主题，或者定义您自己的自定义主题。

默认情况下，CodeTutor 使用我们自己的 `codetutor` 主题。
...
CodeTutor 支持灵活的基于 JSON 的主题系统，让用户可以轻松创建和自定义主题。
...
3. **项目根目录** — 定义在 `<project-root>/.codetutor/themes/*.json`
4. **当前工作目录** — 定义在 `./.codetutor/themes/*.json`
