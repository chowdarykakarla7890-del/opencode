...
Agent skills omogucavaju CodeTutor da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
CodeTutor pretrazuje ove lokacije:

- Konfiguracija projekta: `.codetutor/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, CodeTutor ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.codetutor/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.
...
Kreirajte `.codetutor/skills/git-release/SKILL.md` ovako:
...
CodeTutor navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `codetutor.json`:
...
**Za ugradene agente** (u `codetutor.json`):
