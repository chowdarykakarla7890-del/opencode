...
Du kan legge til eksterne verktøy i CodeTutor ved å bruke _Model Context Protocol_ eller MCP. CodeTutor støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [CodeTutor Config](https://codetutor-docs.vercel.app/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="codetutor.jsonc" {6}
{
...
```jsonc title="codetutor.jsonc" {15}
{
  "$schema": "https://codetutor-docs.vercel.app/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
CodeTutor håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil CodeTutor:
...
Hvis serveren krever autentisering, vil CodeTutor be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `codetutor mcp auth <server-name>`.
...
```json title="codetutor.json" {7-11}
{
  "$schema": "https://codetutor-docs.vercel.app/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
codetutor mcp auth my-oauth-server
```
...
```bash
codetutor mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil CodeTutor lagre tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
codetutor mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i CodeTutor, sammen med innebygde verktøy. Så du kan administrere dem gjennom CodeTutor-konfigurasjonen som alle andre verktøy.
...
```json title="codetutor.json" {11, 14-18}
{
  "$schema": "https://codetutor-docs.vercel.app/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
codetutor mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble CodeTutor til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver CodeTutor-samtale oppført som en separat økt.
