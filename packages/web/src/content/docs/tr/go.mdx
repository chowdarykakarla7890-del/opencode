...
CodeTutor Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, CodeTutor'daki diğer sağlayıcılar gibi çalışır. CodeTutor Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve CodeTutor'u kullanmak için bunu kullanmanıza gerek yoktur.
...
:::tip
CodeTutor ile iyi çalışan seçili bir model ve sağlayıcı grubunu test ettik.
:::
...
CodeTutor Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

CodeTutor Go, CodeTutor'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>OpenCode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `CodeTutor Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye CodeTutor Go'ya abone olabilir.
:::
...
CodeTutor Go aşağıdaki limitleri içerir:
...
CodeTutor yapılandırmanızdaki [model id](/docs/config/#models) formatı `opencode-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `opencode-go/kimi-k3` kullanmalısınız.
...
CodeTutor Go'yu şu amaçlarla oluşturduk:
...
4. CodeTutor ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
