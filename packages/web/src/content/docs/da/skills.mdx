...
Agentferdigheter lar CodeTutor oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
CodeTutor søker etter disse stedene:

- Prosjektkonfigurasjon: `.codetutor/skills/<name>/SKILL.md`
...
For prosjektlokale stier går CodeTutor opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.codetutor/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.
...
Lag `.codetutor/skills/git-release/SKILL.md` slik:
...
CodeTutor viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `codetutor.json`:
...
**For innebygde agenter** (i `codetutor.json`):
