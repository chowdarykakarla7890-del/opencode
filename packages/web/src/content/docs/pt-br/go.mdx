...
O CodeTutor Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no CodeTutor. Você assina o CodeTutor Go e
...
:::tip
Testamos um grupo selecionado de modelos e provedores que funcionam bem com o CodeTutor.
:::
...
O CodeTutor Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O CodeTutor Go funciona como qualquer outro provedor no CodeTutor.

1. Você entra no **<a href={console}>OpenCode Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `CodeTutor Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o CodeTutor Go.
:::
...
O CodeTutor Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do CodeTutor
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o CodeTutor também.
