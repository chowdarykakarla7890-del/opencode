...
Create `.codetutor/commands/test.md`:
...
You can add custom commands through the CodeTutor config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your CodeTutor [config](/docs/config):
...
- Per-project: `.codetutor/commands/`
...
codetutor includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
