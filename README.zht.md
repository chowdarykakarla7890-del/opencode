...
# 套件管理員
npm i -g codetutor-ai@latest        # 也可使用 bun/pnpm/yarn
scoop install codetutor             # Windows
choco install codetutor             # Windows
brew install anomalyco/tap/codetutor # macOS 與 Linux（推薦，始終保持最新）
brew install codetutor              # macOS 與 Linux（官方 brew formula，更新頻率較低）
sudo pacman -S codetutor            # Arch Linux (Stable)
paru -S codetutor-bin               # Arch Linux (Latest from AUR)
mise use -g codetutor               # 任何作業系統
nix run nixpkgs#codetutor           # 或使用 github:anomalyco/codetutor 以取得最新開發分支
```
...
CodeTutor 也提供桌面版應用程式。您可以直接從 [發佈頁面 (releases page)](https://github.com/chowdarykakarla7890-del/opencode/releases) 或 [codetutor.ai/download](https://github.com/chowdarykakarla7890-del/opencode/releases) 下載。
...
1. `$CODETUTOR_INSTALL_DIR` - 自定義安裝目錄
2. `$XDG_BIN_DIR` - 符合 XDG 基礎目錄規範的路徑
3. `$HOME/bin` - 標準使用者執行檔目錄 (若存在或可建立)
4. `$HOME/.codetutor/bin` - 預設備用路徑
...
CodeTutor 內建了兩種 Agent，您可以使用 `Tab` 鍵快速切換。
...
此外，CodeTutor 還包含一個 **general** 子 Agent，用於處理複雜搜尋與多步驟任務。此 Agent 供系統內部使用，亦可透過在訊息中輸入 `@general` 來呼叫。

了解更多關於 [Agents](https://codetutor-docs.vercel.app/docs/agents) 的資訊。

### 線上文件

關於如何設定 CodeTutor 的詳細資訊，請參閱我們的 [**官方文件**](https://codetutor-docs.vercel.app/docs)。

### 參與貢獻

如果您有興趣參與 CodeTutor 的開發，請在提交 Pull Request 前先閱讀我們的 [貢獻指南 (Contributing Docs)](./CONTRIBUTING.md)。

### 基於 CodeTutor 進行開發

如果您正在開發與 CodeTutor 相關的專案，並在名稱中使用了 "codetutor"（例如 "codetutor-dashboard" 或 "codetutor-mobile"），請在您的 README 中加入聲明，說明該專案並非由 CodeTutor 團隊開發，且與我們沒有任何隸屬關係。

---

**加入我們的社群** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=52ao9352-5623-4fa0-b7dd-3407c392c1af&qr_code=true) | [X.com](https://x.com/codetutor)
