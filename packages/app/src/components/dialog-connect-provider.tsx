...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "opencode-go") return language.t("dialog.provider.codetutorGo.tagline")
    return undefined
  }
      ...
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <Show when={i.id === "opencode"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.codetutor.tagline")}</div>
          </Show>
                          ...
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "opencode"
                                ? "dialog.provider.codetutor.tagline"
                                : "dialog.provider.codetutorGo.tagline",
                            )}
                          </span>
        ...
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          ...
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.codetutorZen.line1")}</div>
              <div>{language.t("provider.connect.codetutorZen.line2")}</div>
              <div>
                {language.t("provider.connect.codetutorZen.visit.prefix")}
                <ExternalLink
                  href="https://opencode.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.codetutorZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.codetutorZen.visit.suffix")}
              </div>
            </div>
          </Show>
        ...
        <Switch>
          <Match when={provider().id === "opencode"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.codetutorZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.codetutorZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.codetutorZen.visit.prefix")}
