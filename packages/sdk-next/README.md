...
Effect-native scoped CodeTutor host for in-process applications. This transitional package will replace the existing generated `@opencode-ai/sdk` after its consumers migrate.

The SDK executes Server's assembled HTTP router in memory. It opens no listener and performs no network I/O, while preserving the same routing, middleware, handlers, codecs, and errors as the network client.

```ts
import { CodeTutor } from "@opencode-ai/sdk-next"
...
yield * program.pipe(Effect.provide(CodeTutor.layer))
```

`CodeTutor.layer` adapts `CodeTutor.create()` for dependency injection; it does not define another host implementation.
