...
`../llm.ts` is the CodeTutor session LLM service. It owns CodeTutor concerns: auth, config, model/provider resolution, plugins, permissions, telemetry headers, and runtime selection. It is the only file in this area that should know about the full session request shape.
...
- `CODETUTOR_EXPERIMENTAL_NATIVE_LLM=true` or the umbrella `CODETUTOR_EXPERIMENTAL=true` opts in. Native is not a global replacement.
