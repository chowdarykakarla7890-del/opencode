---
title: Serwer
description: Interakcja z serwerem codetutor za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `codetutor serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient opencode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `codetutor.local` |
...
Ustaw `CODETUTOR_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `codetutor` lub ustaw `CODETUTOR_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `codetutor serve`, jak i `codetutor web`.
...
Kiedy uruchomisz `codetutor`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera codetutor do programowej interakcji z opencode.
:::

Ta architektura umożliwia obsługę wielu klientów przez codetutor i programową interakcję z kodem otwartym.

Możesz uruchomić `codetutor serve`, aby uruchomić samodzielny serwer. Jeśli masz
opencode TUI działa, `codetutor serve` uruchomi nowy serwer.
...
Serwer codetutor udostępnia następujące interfejsy API.
