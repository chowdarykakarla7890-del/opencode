...
- macOS만: `CodeTutor` 메뉴 -> **웹뷰 **(UI가 공백/frozen인 경우).
...
-**macOS/리눅스**: `~/.config/opencode/codetutor.jsonc` (또는 `~/.config/opencode/codetutor.json`) -**macOS/Linux** (외부 설치): `~/.local/share/opencode/codetutor.jsonc`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.config\opencode\codetutor.jsonc`
...
`codetutor.json(c)`가 `server` 섹션을 포함하면 일시적으로 제거하고 데스크톱 앱을 다시 시작합니다.

### 환경 변수 확인

`CODETUTOR_PORT`가 있는 경우, 데스크탑 앱은 로컬 서버의 포트를 사용하려고 합니다.

- `CODETUTOR_PORT` (또는 무료 포트를 선택) 및 재시작.
...
3. 당신은 `codetutor upgrade`를 가진 최신 버전이 있는 것을 지킵니다
...
당신이 액세스 할 수있는 모델을 파악하려면, `codetutor models`를 실행
