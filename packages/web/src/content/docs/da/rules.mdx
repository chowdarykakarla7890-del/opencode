...
Du kan gi tilpassede instruksjoner for at CodeTutor ved at oprete en `AGENTS.md`-fil. Dette ligner på Cursors regler. Den inneholder instruksjoner som vil bli inkludert i LLMs kontekst for at tilpasse oppførselen til ditt spesifikke prosjekt.
...
Dette vil skanne prosjektet og alt dets innhold for at forstå hva prosjektet handler om og generere en `AGENTS.md`-fil med det. Dette hjelper codetutor at navigere i prosjektet bedre.
...
For brugere som migrerer fra Claude Code, støtter CodeTutor Claude Codes filkonvensjoner som fallbacks:
...
Du kan spesifisere egendefinerte instruksjonsfiler i din `codetutor.json` eller den globale `~/.config/opencode/codetutor.json`. Dette lar deg og teamet ditt gjenbruge eksisterende regler i stedet for at måtte duplisere dem til AGENTS.md.
...
### Brug af codetutor.json

Den anbefalte tilnærmingen er at bruge feltet `instructions` i `codetutor.json`:
...
Du kan lære codetutor at lese eksterne filer ved at gi eksplisitte instruksjoner i `AGENTS.md`. Her er et praktisk eksempel:
...
- Sørg for at codetutor laster filer kun når det er nødvendig for den spesifikke oppgaven

:::tip
For monorepos eller prosjekter med delte standarder er brug av `codetutor.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
