...
import type { CodeTutorEvent } from "../src"

test("embedded client uses the real router and handlers", async () => {
  const directory = await mkdtemp(join(tmpdir(), "opencode-embedded-"))
  const database = Flag.CODETUTOR_DB
  Flag.CODETUTOR_DB = join(directory, "opencode.sqlite")
  const { AbsolutePath, Agent, Location, Model, CodeTutor, Prompt, Provider, Session, Tool } = await import("../src")
  const sessionID = Session.ID.make(`ses_embedded_${crypto.randomUUID()}`)
  const model = Model.Ref.make({ id: Model.ID.make("embedded"), providerID: Provider.ID.make("test") })

  try {
    const program = Effect.gen(function* () {
      const opencode = yield* CodeTutor.create()
...
test("Location-owned runner events reach the ready global client", async () => {
  ...
  const database = Flag.CODETUTOR_DB
  Flag.CODETUTOR_DB = join(directory, "opencode.sqlite")
  const { AbsolutePath, Location, CodeTutor, Prompt, Session } = await import("../src")
  const sessionID = Session.ID.make(`ses_embedded_${crypto.randomUUID()}`)
  ...
  try {
    const program = Effect.gen(function* () {
      ...
      const prompted = yield* Deferred.make<CodeTutorEvent>()
...
test("independent embedded hosts do not share live notifications", async () => {
  ...
  const database = Flag.CODETUTOR_DB
  Flag.CODETUTOR_DB = join(directory, "opencode.sqlite")
  const { AbsolutePath, Agent, Location, CodeTutor, Session } = await import("../src")
  const sessionID = Session.ID.make(`ses_embedded_${crypto.randomUUID()}`)

  try {
    const program = Effect.gen(function* () {
      const first = yield* CodeTutor.create()
      const second = yield* CodeTutor.create()
      ...
      const observe = (ready: Latch.Latch, event: Latch.Latch) =>
        Stream.runForEach((notification: CodeTutorEvent) =>
...
test("embedded client is available as a Layer service", async () => {
  ...
  const database = Flag.CODETUTOR_DB
  Flag.CODETUTOR_DB = join(directory, "opencode.sqlite")
  const { AbsolutePath, Location, CodeTutor, Session } = await import("../src")
  const sessionID = Session.ID.make(`ses_embedded_${crypto.randomUUID()}`)

  try {
    const created = await Effect.runPromise(
      Effect.gen(function* () {
        const opencode = yield* CodeTutor.Service
        return yield* opencode.sessions.create({
          id: sessionID,
          location: Location.Ref.make({ directory: AbsolutePath.make(directory) }),
        })
      }).pipe(Effect.provide(CodeTutor.layer), Effect.scoped),
    )
