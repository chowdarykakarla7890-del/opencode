---
title: Policies
description: Control which configured resources CodeTutor may use.
---

Policies control whether CodeTutor may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `codetutor.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether CodeTutor may use a resource such as an LLM provider.
...
CodeTutor currently supports one policy action:
