...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".codetutor"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".codetutor", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".codetutor", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.CODETUTOR_PURE ? [] : pluginOrigins
    if (Flag.CODETUTOR_PURE && pluginOrigins.length) {
    }
