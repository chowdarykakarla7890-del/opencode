---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i CodeTutor.
---

import config from "../../../../config.mjs"
export const console = config.console

CodeTutor bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i CodeTutor-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i CodeTutor-konfigurationen.
...
OpenCode Zen er en liste over modeller leveret af CodeTutor-teamet, som er blevet testet og verificeret til at fungere godt med CodeTutor. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med OpenCode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg codetutor og gå til [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/auth).
...
Det fungerer som alle andre udbydere i CodeTutor og er helt valgfrit at bruge.

---

## CodeTutor Go

CodeTutor Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af CodeTutor-teamet, som er testet og verificeret til at fungere godt med CodeTutor.

1. Kør kommandoen `/connect` i TUI, vælg `CodeTutor Go`, og gå til [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/zen).
...
Sådan bruger du Amazon Bedrock med CodeTutor:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `codetutor.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i CodeTutor understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere codetutor til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at codetutor skal fungere korrekt.
   :::
...
gemme tokenet i codetutor auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
CodeTutor bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse CodeTutor
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`codetutor.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med CodeTutor:
...
Sådan bruger du Google Vertex AI med CodeTutor:
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver CodeTutor-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver CodeTutor-samtale opført som en separat session.
...
Du kan konfigurere codetutor til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere codetutor til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere codetutor til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for CodeTutor. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med CodeTutor:
...
3. Kopier API-nøglen til brug i CodeTutor.
...
6. **Vigtigt**: Før du bruger skymodeller i CodeTutor, skal du hente modelinformationen lokalt:
...
OpenCode Zen er en liste over testede og verificerede modeller leveret af CodeTutor-teamet. [Læs mere](/docs/zen).
...
3. Indtast CodeTutor API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med CodeTutor:
...
4. Opret eller opdater `codetutor.json`-filen i projektkataloget:
...
`limit`-felterne lader CodeTutor forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `codetutor auth list` for at se om legitimationsoplysningerne
