...
description: Skriv dine egne programtillegg for å utvide CodeTutor.
---

Plugins lar deg utvide CodeTutor ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til CodeTutor.
...
- `.codetutor/plugins/` - Programtillegg på prosjektnivå
...
1. Global konfigurasjon (`~/.config/opencode/codetutor.json`)
2. Prosjektkonfigurasjon (`codetutor.json`)
3. Global plugin-katalog (`~/.config/opencode/plugins/`)
4. Prosjektpluginkatalog (`.codetutor/plugins/`)
...
CodeTutor kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En CodeTutor SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide CodeTutor.
...
:::note
Hvis du bruker CodeTutor-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre CodeTutor fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til CodeTutor:
...
`tool`-hjelperen lager et tilpasset verktøy som CodeTutor kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for CodeTutor sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://codetutor-docs.vercel.app/docs/sdk) for detaljer.
