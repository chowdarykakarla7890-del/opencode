...
Per diagnosticare problemi con CodeTutor, inizia controllando i log e i dati locali che salva su disco.
...
codetutor salva i dati delle sessioni e altri dati dell'applicazione su disco in:
...
CodeTutor Desktop esegue in background un server locale di CodeTutor (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `CodeTutor` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/opencode/codetutor.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opencode\codetutor.jsonc`
...
CodeTutor puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che CodeTutor possa ricostruirla.

1. Chiudi completamente CodeTutor Desktop.
...
3. Riavvia CodeTutor Desktop.

---

### Risolvi problemi di connessione al server

CodeTutor Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `codetutor.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `CODETUTOR_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `CODETUTOR_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, CodeTutor Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di CodeTutor.

---

### Notifiche non visualizzate

CodeTutor Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per CodeTutor nelle impostazioni del sistema operativo, e
...
1. Chiudi CodeTutor Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app CodeTutor Desktop):
...
Se riscontri problemi con CodeTutor:
...
Per capire a quali modelli hai accesso, esegui `codetutor models`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. codetutor installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
2. Riavvia codetutor per reinstallare i pacchetti provider piu recenti

Questo forzera codetutor a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
codetutor rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
