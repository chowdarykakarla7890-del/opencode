...
Agent Skills erlauben CodeTutor, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
CodeTutor sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft CodeTutor vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.codetutor/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.
...
Erstelle `.codetutor/skills/git-release/SKILL.md` zum Beispiel so:
...
CodeTutor listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `codetutor.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `codetutor.json`):
