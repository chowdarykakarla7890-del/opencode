...
Para usar o codetutor no seu terminal, você precisará de:
...
- **Usando Homebrew no macOS e Linux**
  ...
  > Recomendamos usar o tap do codetutor para as versões mais atualizadas. A fórmula oficial `brew install opencode` é mantida pela equipe do Homebrew e é atualizada com menos frequência.
...
O suporte para instalar o codetutor no Windows usando Bun está atualmente em andamento.

Você também pode baixar o binário na seção [Releases](https://github.com/chowdarykakarla7890-del/opencode/releases).
...
1. Execute o comando `/connect` no TUI, selecione codetutor e acesse [codetutor-docs.vercel.app/auth](https://codetutor-docs.vercel.app/auth).
...
Em seguida, inicialize o codetutor para o projeto executando o seguinte comando.

```bash frame="none"
/init
```

Isso fará com que o codetutor analise seu projeto e crie um arquivo `AGENTS.md` na raiz do projeto.

:::tip
Você deve commitar o arquivo `AGENTS.md` do seu projeto no Git.
:::

Isso ajuda o codetutor a entender a estrutura do projeto e os padrões de codificação utilizados.

---

## Uso

Agora você está pronto para usar o codetutor para trabalhar em seu projeto. Sinta-se à vontade para perguntar qualquer coisa!
...
Você pode pedir ao codetutor para explicar a base de código para você.
...
Você pode pedir ao codetutor para adicionar novos recursos ao seu projeto. Embora primeiro recomendemos pedir para ele criar um plano.

1. **Criar um plano**

   O codetutor tem um _Modo de Plano_ que desabilita sua capacidade de fazer alterações e, em vez disso, sugere _como_ implementará o recurso.
   ...
   Você quer dar ao codetutor detalhes suficientes para entender o que você deseja. Ajuda conversar com ele como se você estivesse falando com um desenvolvedor júnior da sua equipe.

   :::tip
   Dê ao codetutor bastante contexto e exemplos para ajudá-lo a entender o que você deseja.
   :::
...
2. **Iterar sobre o plano**
   ...
   O codetutor pode escanear qualquer imagem que você fornecer e adicioná-la ao prompt. Você pode fazer isso arrastando e soltando uma imagem no terminal.
...
Para alterações mais simples, você pode pedir ao codetutor para construí-las diretamente sem precisar revisar o plano primeiro.
...
Você quer ter certeza de fornecer uma boa quantidade de detalhes para que o codetutor faça as alterações corretas.

---

### Desfazendo alterações

Vamos supor que você peça ao codetutor para fazer algumas alterações.
...
O codetutor agora reverterá as alterações que você fez e mostrará sua mensagem original novamente.

```txt frame="none" "@packages/functions/src/api/index.ts"
Can you refactor the function in @packages/functions/src/api/index.ts?
```

A partir daqui, você pode ajustar o prompt e pedir ao codetutor para tentar novamente.
...
As conversas que você tem com o codetutor podem ser [compartilhadas com sua equipe](/docs/share).
...
Aqui está uma [conversa de exemplo](https://codetutor-docs.vercel.app/s/4XP1fce5) com o opencode.
