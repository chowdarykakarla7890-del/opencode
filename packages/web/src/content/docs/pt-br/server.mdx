---
title: Servidor
description: Interaja com o servidor codetutor via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `codetutor serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente codetutor pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `codetutor.local` |
...
Defina `CODETUTOR_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `codetutor`, ou defina `CODETUTOR_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `codetutor serve` quanto ao `codetutor web`.
...
Quando você executa `codetutor`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor codetutor para interagir com o codetutor programaticamente.
:::

Essa arquitetura permite que o codetutor suporte múltiplos clientes e permite que você interaja com o codetutor programaticamente.

Você pode executar `codetutor serve` para iniciar um servidor autônomo. Se você tiver o TUI do codetutor em execução, `codetutor serve` iniciará um novo servidor.
...
O servidor codetutor expõe as seguintes APIs.
