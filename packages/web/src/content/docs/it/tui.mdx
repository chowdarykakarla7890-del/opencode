---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di CodeTutor.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CodeTutor fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire CodeTutor avvia la TUI per la directory corrente.
...
Quando usi la TUI di CodeTutor, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a CodeTutor. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da CodeTutor. _Alias_: `/quit`, `/q`
...
Questo è separato da `codetutor.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo CodeTutor attende dopo il leader key. Default: `2000`.
