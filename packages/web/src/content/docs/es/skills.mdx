...
Las habilidades del agente permiten a CodeTutor descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
CodeTutor busca estas ubicaciones:

- Configuración del proyecto: `.codetutor/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, CodeTutor sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.codetutor/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.
...
Crea `.codetutor/skills/git-release/SKILL.md` así:
...
CodeTutor enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `codetutor.json`:
...
**Para agentes integrados** (en `codetutor.json`):
