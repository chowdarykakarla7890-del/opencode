...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.codetutor.desktop.dev",
  beta: "ai.codetutor.desktop.beta",
  prod: "ai.codetutor.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.codetutor.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `codetutor.settings.dat` is special: it maps to the `codetutor.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"codetutor.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // codetutor.settings.dat → the electron settings store ("codetutor.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "codetutor.global.dat").
  const storeName = filename === "codetutor.settings.dat" ? "codetutor.settings" : filename
