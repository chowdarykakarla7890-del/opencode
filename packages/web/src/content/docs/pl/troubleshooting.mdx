...
codetutor przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:
...
- Tylko macOS: menu `CodeTutor` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/opencode/codetutor.jsonc` (lub `~/.config/opencode/codetutor.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/opencode/codetutor.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencode\codetutor.jsonc`
...
codetutor może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby codetutor mógł ją odbudować.
...
Jeśli Twój `codetutor.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `CODETUTOR_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `CODETUTOR_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
- powiadomienia są włączone dla codetutor w ustawieniach systemu operacyjnego, oraz
...
3. Upewnij się, że masz najnowszą wersję z `codetutor upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `codetutor models`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. codetutor dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
Zmusi to codetutor do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
codetutor wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
