...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-codetutor-directory", directory)
    ...
    Effect.gen(function* () {
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          body: JSON.stringify({ $schema: "https://codetutor-docs.vercel.app/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-codetutor-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-codetutor-workspace")
