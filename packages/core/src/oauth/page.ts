...
// etc.).
//
// The visual language mirrors the CodeTutor app: the design tokens are a curated
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `CodeTutor is now connected to ${escapeHtml(provider)}.` : "CodeTutor is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `CodeTutor couldn't finish connecting to ${escapeHtml(provider)}.`
        : "CodeTutor couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from CodeTutor.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · CodeTutor</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("CodeTutor couldn't finish connecting to "+PROVIDER+"."):"CodeTutor couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from CodeTutor."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("CodeTutor is now connected to "+PROVIDER+"."):"CodeTutor is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// CodeTutor wordmark — same path geometry as packages/ui/src/components/logo.tsx (Logo).
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 234 42" fill="none" aria-label="CodeTutor" role="img">
