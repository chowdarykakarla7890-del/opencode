...
Umiejętności agenta pozwalają codetutor odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
codetutor przeszukuje te lokalizacje:

- Project config: `.codetutor/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu codetutor przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.codetutor/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.
...
Utwórz `.codetutor/skills/git-release/SKILL.md` w ten sposób:
...
codetutor wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `codetutor.json`:
...
**Dla agentów wbudowanych** (w `codetutor.json`):
