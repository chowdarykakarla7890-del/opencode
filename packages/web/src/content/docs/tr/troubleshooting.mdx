...
codetutor ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
opencode Desktop arka planda yerel bir codetutor sunucusu (`opencode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `codetutor` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/opencode/codetutor.jsonc` (veya `~/.config/opencode/codetutor.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/opencode/codetutor.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\opencode\codetutor.jsonc` yapistirin
...
codetutor diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:
...
`codetutor.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `CODETUTOR_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `CODETUTOR_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, codetutor ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.
...
- Isletim sistemi ayarlarinizda codetutor icin bildirimler etkinse
...
codetutor ile ilgili bir sorun yasiyorsaniz:
...
Erisiminiz olan modelleri gormek icin `codetutor models` calistirin.
