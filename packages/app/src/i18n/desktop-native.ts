...
export function detectDesktopNativeLocale(languages: readonly string[]): DesktopNativeLocale {
  for (const language of languages) {
    ...
    const match = DESKTOP_NATIVE_LOCALES.find((candidate) => {
      ...
      const sourceScript = source.script === "Aran" ? "Arab" : source.script
      return target?.language === source.language && target.script === sourceScript
    })
...
export const DESKTOP_NATIVE_ENGLISH = {
  ...
  "desktop.recovery.loadFailed": "CodeTutor failed to load",
  "desktop.recovery.terminated": "CodeTutor window terminated unexpectedly",
  "desktop.recovery.unresponsive": "CodeTutor is not responding",
  ...
  "desktop.wsl.error.installOpencode": "CodeTutor installation failed",
  "desktop.wsl.error.alreadyAdded": "{{distro}} is already added",
  "desktop.wsl.error.codetutorMissing": "opencode is not installed in this distro",
  "desktop.wsl.error.codetutorCannotRun": "opencode is installed but could not run",
  "desktop.wsl.error.codetutorNotInstalled": "CodeTutor is not installed in {{distro}}",
  "desktop.wsl.error.updateVersion":
    "CodeTutor update finished but {{distro}} still reports {{installed}}; expected {{expected}}",
