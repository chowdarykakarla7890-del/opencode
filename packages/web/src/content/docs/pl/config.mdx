---
title: Konfiguracja
description: Ustawienia CodeTutor w pliku JSON.
---

Możesz dostosować CodeTutor za pomocą pliku konfiguracyjnego JSON.

---

## Format

CodeTutor obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
2. **Konfiguracja globalna** (`~/.config/opencode/codetutor.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `CODETUTOR_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`codetutor.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.codetutor`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `CODETUTOR_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.codetutor` i `~/.config/opencode` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Umieść swoją globalną konfigurację CodeTutor w `~/.config/opencode/codetutor.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.
...
Dodaj `codetutor.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy CodeTutor się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `CODETUTOR_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `CODETUTOR_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.codetutor` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.codetutor`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`codetutor-docs.vercel.app/config.json`**](https://codetutor-docs.vercel.app/config.json).
...
Użyj `CODETUTOR_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `codetutor.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `codetutor serve` i `codetutor web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer CodeTutor.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `codetutor.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie CodeTutor wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/opencode/agents/` lub `.codetutor/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, CodeTutor powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`codetutor run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/opencode/commands/` lub `.codetutor/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
CodeTutor może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy CodeTutor nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie CodeTutor **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają CodeTutor o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.codetutor/plugins/` lub `~/.config/opencode/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie CodeTutor tylko do określonych dostawców.
