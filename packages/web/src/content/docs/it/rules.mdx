...
Puoi fornire istruzioni personalizzate a codetutor creando un file `AGENTS.md`. E' simile alle regole di Cursor. Contiene istruzioni che verranno incluse nel contesto dell'LLM per personalizzarne il comportamento per il tuo progetto.
...
Questo scansiona il progetto e tutti i suoi contenuti per capire di cosa si tratta e generare un file `AGENTS.md`. Questo aiuta codetutor a orientarsi meglio nel progetto.
...
codetutor supporta anche la lettura del file `AGENTS.md` da piu' posizioni, e questo serve a scopi diversi.
...
Per gli utenti che migrano da Claude Code, CodeTutor supporta come fallback le convenzioni di file di Claude Code:
...
Quando codetutor si avvia, cerca i file di regole in questo ordine:
...
Puoi specificare file di istruzioni personalizzati nel tuo `codetutor.json` o nel globale `~/.config/opencode/codetutor.json`. Questo permette a te e al tuo team di riusare regole esistenti invece di doverle duplicare in AGENTS.md.
...
Anche se codetutor non interpreta automaticamente i riferimenti a file in `AGENTS.md`, puoi ottenere una funzionalita' simile in due modi:

### Usa codetutor.json

L'approccio consigliato e' usare il campo `instructions` in `codetutor.json`:
...
Puoi insegnare a codetutor a leggere file esterni fornendo istruzioni esplicite nel tuo `AGENTS.md`. Ecco un esempio pratico:
...
- Assicurati che codetutor carichi file solo quando servono per il task specifico

:::tip
Per monorepo o progetti con standard condivisi, usare `codetutor.json` con pattern glob (come `packages/*/AGENTS.md`) e' piu' manutenibile rispetto alle istruzioni manuali.
:::
