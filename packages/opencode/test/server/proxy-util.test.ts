...
describe("ProxyUtil", () => {
  ...
  describe("headers", () => {
    ...
    test("strips opencode-specific headers", () => {
      const req = new Request("http://localhost", {
        headers: {
          "x-codetutor-directory": "/home/user/project",
          "x-codetutor-workspace": "ws_123",
          "accept-encoding": "gzip",
          "x-custom": "keep",
        },
      })
      const result = ProxyUtil.headers(req)
      expect(result.get("x-codetutor-directory")).toBeNull()
      expect(result.get("x-codetutor-workspace")).toBeNull()
