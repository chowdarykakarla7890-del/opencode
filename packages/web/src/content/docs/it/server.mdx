---
title: Server
description: Interagisci con il server codetutor via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `codetutor serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client opencode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `codetutor.local` |
...
Imposta `CODETUTOR_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `codetutor`, oppure imposta `CODETUTOR_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `codetutor serve` sia per `codetutor web`.
...
Quando esegui `codetutor` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server codetutor per interagire con codetutor in modo programmatico.
:::

Questa architettura permette a codetutor di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `codetutor serve` per avviare un server standalone. Se la TUI di codetutor e gia in esecuzione, `codetutor serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di CodeTutor.
...
Il server codetutor espone le seguenti API.
