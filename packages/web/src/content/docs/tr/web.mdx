...
:::caution
`CODETUTOR_SERVER_PASSWORD` ayarlanmadıysa sunucunun güvenliği kaldırılacaktır. Bu, yerel kullanım için iyidir ancak ağ erişimi için ayarlanmalıdır.
:::

:::tip[Windows Kullanıcıları]
En iyi deneyim için PowerShell yerine `codetutor web`'yi [WSL](/docs/windows-wsl)'den çalıştırın. Bu, uygun dosya sistemi erişimini ve terminal entegrasyonunu sağlar.
:::
...
codetutor varsayılan olarak kullanılabilir bir bağlantı noktasını seçer. Bir bağlantı noktası belirtebilirsiniz:
...
`0.0.0.0` kullanıldığında, codetutor hem yerel hem de ağ adreslerini gösterecektir:
...
Bu, ana bilgisayar adını otomatik olarak `0.0.0.0` olarak ayarlar ve sunucuyu `codetutor.local` olarak tanıtır.
...
Erişimi korumak için `CODETUTOR_SERVER_PASSWORD` ortam değişkenini kullanarak bir parola ayarlayın:

```bash
CODETUTOR_SERVER_PASSWORD=secret codetutor web
```

Kullanıcı adı varsayılan olarak `codetutor` şeklindedir ancak `CODETUTOR_SERVER_USERNAME` ile değiştirilebilir.

---

## Web Arayüzünü Kullanma

Web arayüzü başlatıldığında codetutor oturumlarınıza erişim sağlar.
...
Sunucu ayarlarını `codetutor.json` yapılandırma dosyanızda da yapılandırabilirsiniz:
