    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.CODETUTOR_CHANNEL === "prod" &&
...
const deepLinkEvent = "codetutor:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__CODETUTOR__ ??= {}
  const pending = window.__CODETUTOR__.deepLinks ?? []
  window.__CODETUTOR__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `codetutor.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, onClick) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://codetutor-docs.vercel.app/favicon.svg",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("codetutor.global.dat").getItem("language")
