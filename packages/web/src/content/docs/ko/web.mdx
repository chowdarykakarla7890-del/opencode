---
title: Web
description: 브라우저에서 CodeTutor 사용하기.
---
...
:::caution
`CODETUTOR_SERVER_PASSWORD`가 설정되지 않은 경우 서버가 중단되지 않습니다. 이것은 로컬 사용을위한 괜찮지만 네트워크 액세스를 설정해야합니다.
:::

:::tip[Windows 사용자]
최고의 경험을 위해서는 PowerShell보다 `codetutor web`를 실행하십시오. 적절한 파일 시스템 액세스 및 terminal 통합을 보장합니다.
:::
...
이 자동으로 `0.0.0.0`에 hostname을 설정하고 `codetutor.local`로 서버를 광고합니다.
...
액세스를 보호하려면 `CODETUTOR_SERVER_PASSWORD` 환경 변수를 사용하여 암호를 설정하십시오.

```bash
CODETUTOR_SERVER_PASSWORD=secret codetutor web
```

`codetutor`에 대한 사용자의 기본값은 `CODETUTOR_SERVER_USERNAME`로 변경할 수 있습니다.
...
`codetutor.json` config 파일에서 서버 설정을 구성할 수 있습니다:
