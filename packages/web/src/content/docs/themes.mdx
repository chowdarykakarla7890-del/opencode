...
With CodeTutor you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, CodeTutor uses our own `codetutor` theme.
...
CodeTutor comes with several built-in themes.
...
- Want CodeTutor to match their terminal's appearance
...
CodeTutor supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
3. **Project root directory** - Defined in the `<project-root>/.codetutor/themes/*.json`
4. **Current working directory** - Defined in `./.codetutor/themes/*.json`
