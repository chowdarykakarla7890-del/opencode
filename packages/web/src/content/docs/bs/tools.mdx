...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. CodeTutor dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u CodeTutor.
...
:::note
Ovaj alat je dostupan samo kada je `CODETUTOR_EXPERIMENTAL_LSP_TOOL=true` (ili `CODETUTOR_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz CodeTutor provajdera ili kada je varijabla `CODETUTOR_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju CodeTutor:
