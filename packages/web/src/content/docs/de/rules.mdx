---
title: Regeln
description: Lege eigene Anweisungen fuer codetutor fest.
---

Du kannst codetutor ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in codetutor den Befehl `/init` aus.
...
So kann codetutor sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht codetutor sie zu erweitern.
...
codetutor kann `AGENTS.md` aus mehreren Orten lesen.
...
Wenn du von Claude Code kommst, unterstuetzt CodeTutor dessen Dateikonventionen als Fallback:
...
Beim Start sucht codetutor in dieser Reihenfolge nach Regeldateien:
...
Du kannst zusaetzliche Anweisungsdateien in deiner `codetutor.json` oder in `~/.config/opencode/codetutor.json` angeben.
...
codetutor parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit codetutor.json

Empfohlen ist das `instructions`-Feld in der `codetutor.json`:
...
Du kannst codetutor explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `codetutor.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
