...
codetutor usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.
...
OpenCode Zen é uma lista de modelos fornecidos pela equipe do codetutor que foram testados e verificados para funcionar bem com o opencode. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o OpenCode Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `OpenCode Zen` e acesse [codetutor-docs.vercel.app/zen](https://codetutor-docs.vercel.app/zen).
...
Funciona como qualquer outro provedor no codetutor e é completamente opcional.

---

## CodeTutor Go

CodeTutor Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do codetutor que foram
testados e verificados para funcionar bem com o opencode.

1. Execute o comando `/connect` no TUI, selecione `CodeTutor Go` e acesse [codetutor-docs.vercel.app/zen](https://codetutor-docs.vercel.app/zen).
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `codetutor.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no codetutor não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o codetutor para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o codetutor funcione corretamente.
   :::
...
codetutor usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
...
arquivo `codetutor.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `codetutor.json`:
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automaticamente cada conversa do codetutor como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do codetutor listada como uma sessão separada.
...
Você pode configurar o codetutor para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o codetutor para usar modelos locais através do LM Studio.
...
Você pode configurar o codetutor para usar modelos locais através do Ollama.
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no codetutor.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `codetutor.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o codetutor entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `codetutor auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do codetutor e:
