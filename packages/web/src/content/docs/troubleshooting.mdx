...
To debug issues with CodeTutor, start by checking the logs and local data it stores on disk.
...
codetutor stores session data and other application data on disk at:
...
To uninstall the CodeTutor CLI and remove its related files, run:
...
To remove CodeTutor Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

CodeTutor Desktop runs a local CodeTutor server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `CodeTutor` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/opencode/codetutor.jsonc` (or `~/.config/opencode/codetutor.json`)
- **macOS/Linux** (older installs): `~/.local/share/opencode/codetutor.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opencode\codetutor.jsonc`
...
CodeTutor can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so CodeTutor can rebuild it.

1. Quit CodeTutor Desktop completely.
...
3. Restart CodeTutor Desktop.

---

### Fix server connection issues

CodeTutor Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `codetutor.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `CODETUTOR_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `CODETUTOR_PORT` (or pick a free port) and restart.
...
On Windows, CodeTutor Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with CodeTutor's features.

---

### Notifications not showing

CodeTutor Desktop only shows system notifications when:

- notifications are enabled for CodeTutor in your OS settings, and
...
1. Quit CodeTutor Desktop.
2. Find and delete these files (they live in the CodeTutor Desktop app data directory):
...
If you're experiencing issues with CodeTutor:
...
### CodeTutor won't start

1. Check the logs for error messages
2. Try running with `--print-logs` to see output in the terminal
3. Ensure you have the latest version with `codetutor upgrade`
...
To figure out what models you have access to, run `codetutor models`
...
If you encounter API call errors, this may be due to outdated provider packages. codetutor dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
2. Restart codetutor to reinstall the latest provider packages

This will force codetutor to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
codetutor will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
