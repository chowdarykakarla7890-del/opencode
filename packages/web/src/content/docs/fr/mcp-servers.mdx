...
Vous pouvez ajouter des outils externes à CodeTutor à l'aide du _Model Context Protocol_ ou MCP. CodeTutor prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [CodeTutor Config](https://codetutor-docs.vercel.app/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
CodeTutor gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, CodeTutor :
...
Si le serveur nécessite une authentification, CodeTutor vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `codetutor mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, CodeTutor stockera les jetons en toute sécurité dans `~/.local/share/opencode/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans CodeTutor, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration CodeTutor comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter CodeTutor à votre compte Sentry.
